...
CTFStatsSummaryPanel *GStatsSummaryPanel()
{
	...
	{
		g_pTFStatsSummaryPanel = new CTFStatsSummaryPanel( NULL, "CTFStatsSummaryPanel" );
	}
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CTFStatsSummaryPanel::CTFStatsSummaryPanel( vgui::Panel* parent, const char *panelName ) : vgui::EditablePanel( parent, panelName,
...
void CTFStatsSummaryPanel::SetValueAsClass( const char *pDialogVariable, int iValue, int iPlayerClass )
{
	if ( iValue > 0 )
	{
		...
		wchar_t wzVal[16];
		wchar_t wzMsg[128];
                swprintf_s( wzVal, sizeof(wzVal), L"%d", iValue);
