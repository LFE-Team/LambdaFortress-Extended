...
{
	BaseModHybridButton::EnableCondition mCondition;
	char mConditionName[64];
};

HybridEnableStates sHybridStates[] = 
{
	{ BaseModHybridButton::EC_LIVE_REQUIRED,	"LiveRequired" },
	{ BaseModHybridButton::EC_NOTFORDEMO,		"Never" }
};
...
BaseModHybridButton::BaseModHybridButton( Panel *parent, const char *panelName, const char *text, Panel *pActionSignalTarget, const char *pCmd )
...
BaseModHybridButton::BaseModHybridButton( Panel *parent, const char *panelName, const wchar_t *text, Panel *pActionSignalTarget, const char *pCmd )
	: BaseClass( parent, panelName, text, pActionSignalTarget, pCmd )
{
	SetPaintBorderEnabled( false );
	SetPaintBackgroundEnabled( false );
...
BaseModHybridButton::~BaseModHybridButton()
{
...
Panel* BaseModHybridButton::NavigateUp()
{
	...
	if ( !target && 
		(dynamic_cast< DropDownMenu * >( GetParent() ) || dynamic_cast< CTFAdvSlider * >( GetParent() )) )
	{
