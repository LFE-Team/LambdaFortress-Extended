...
extern ConVar sk_plr_dmg_xbow_hl1_bolt_plr;
extern ConVar sk_plr_dmg_xbow_hl1_bolt_npc;
...
LINK_ENTITY_TO_CLASS( crossbow_bolt, CcrossbowBoltHL1 );

BEGIN_DATADESC( CcrossbowBoltHL1 )
	// Function Pointers
...
CcrossbowBoltHL1 *CcrossbowBoltHL1::BoltCreate( const Vector &vecOrigin, const QAngle &angAngles, CBasePlayer *pentOwner )
{
	// Create a new entity with CcrossbowBoltHL1 private data
	CcrossbowBoltHL1 *pBolt = (CcrossbowBoltHL1 *)CreateEntityByName( "crossbow_bolt" );
...
void CcrossbowBoltHL1::Spawn( )
{
	...
	SetTouch( &CcrossbowBoltHL1::BoltTouch );

	SetThink( &CcrossbowBoltHL1::BubbleThink );
...
void CcrossbowBoltHL1::Precache( )
{
	PrecacheModel( BOLT_MODEL );
	PrecacheScriptSound( "BaseGrenade.Explode" );    
}

void CcrossbowBoltHL1::BoltTouch( CBaseEntity *pOther )
{
	...
	if ( pOther->m_takedamage != DAMAGE_NO )
	{
		...
		if ( pOther->IsPlayer() )
		{
            float m_fDamage = sk_plr_dmg_xbow_hl1_bolt_plr.GetFloat() * g_pGameRules->GetDamageMultiplier();
		...
		{
			CTakeDamageInfo	dmgInfo( this, GetOwnerEntity(), sk_plr_dmg_xbow_hl1_bolt_npc.GetFloat() * g_pGameRules->GetDamageMultiplier(), DMG_BULLET | DMG_NEVERGIB );
			CalculateMeleeDamageForce( &dmgInfo, vecNormalizedVel, tr.endpos );
			pOther->DispatchTraceAttack( dmgInfo, vecNormalizedVel, &tr );
		}
...
//			NDebugOverlay::Box( GetAbsOrigin(), Vector( -16, -16, -16 ), Vector( 16, 16, 16 ), 0, 0, 255, 0, 10 );
	...
	else
	{
		...
		SetThink( &CcrossbowBoltHL1::SUB_Remove );
	...
	if ( g_pGameRules->IsMultiplayer() && m_bExplode )
	{
        SetThink( &CcrossbowBoltHL1::ExplodeThink );
        SetNextThink( gpGlobals->curtime + 0.1f );
	}
}

void CcrossbowBoltHL1::ExplodeThink( void )
{
    //    int iContents = UTIL_PointContents( pev->origin );
    CTakeDamageInfo	dmgInfo( this, GetOwnerEntity(), sk_plr_dmg_xbow_hl1_bolt_npc.GetFloat() * g_pGameRules->GetDamageMultiplier(), DMG_BLAST );
...
void CcrossbowBoltHL1::BubbleThink( void )
{
	QAngle angNewAngles;
...
IMPLEMENT_NETWORKCLASS_ALIASED( WeaponHL1Crossbow, DT_WeaponHL1Crossbow );

BEGIN_NETWORK_TABLE( CWeaponHL1Crossbow, DT_WeaponHL1Crossbow )
#ifdef CLIENT_DLL
...
BEGIN_PREDICTION_DATA( CWeaponHL1Crossbow )
#ifdef CLIENT_DLL
	DEFINE_PRED_FIELD( m_fInZoom, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE ),
#endif
END_PREDICTION_DATA()

LINK_ENTITY_TO_CLASS( weapon_crossbow_hl1, CWeaponHL1Crossbow );

PRECACHE_WEAPON_REGISTER( weapon_crossbow_hl1 );

//IMPLEMENT_SERVERCLASS_ST( CWeaponHL1Crossbow, DT_WeaponHL1Crossbow )
//END_SEND_TABLE()

BEGIN_DATADESC( CWeaponHL1Crossbow )
END_DATADESC()

#if 0
#ifndef CLIENT_DLL

acttable_t	CWeaponHL1Crossbow::m_acttable[] = 
{
...
IMPLEMENT_ACTTABLE(CWeaponHL1Crossbow);
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CWeaponHL1Crossbow::CWeaponHL1Crossbow( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponHL1Crossbow::Precache( void )
{
#ifndef CLIENT_DLL
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponHL1Crossbow::PrimaryAttack( void )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponHL1Crossbow::SecondaryAttack( void )
{
	ToggleZoom();
...
void CWeaponHL1Crossbow::FireBolt( void )
{
	if ( m_iClip1 <= 0 )
	{
		if ( !m_bFireOnEmpty )
...
#ifndef CLIENT_DLL
	CcrossbowBoltHL1 *pBolt = CcrossbowBoltHL1::BoltCreate( vecSrc, angAiming, pOwner );
...
bool CWeaponHL1Crossbow::Reload( void )
{
	bool fRet;
...
void CWeaponHL1Crossbow::WeaponIdle( void )
{
...
bool CWeaponHL1Crossbow::Deploy( void )
{
	if ( m_iClip1 <= 0 )
	{
...
bool CWeaponHL1Crossbow::Holster( CBaseCombatWeapon *pSwitchingTo )
{
	if ( m_fInZoom )
	{
		ToggleZoom();
...
void CWeaponHL1Crossbow::ToggleZoom( void )
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
	if ( !pPlayer )
	{
