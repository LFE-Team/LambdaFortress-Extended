...
ConVar  sk_rgrunt_pellets("sk_rgrunt_pellets", "5");
ConVar  sk_rgrunt_gspeed("sk_rgrunt_gspeed", "600");
...
int g_fRGruntQuestion;				// true if an idle grunt asked a question. Cleared when someone answers.
int g_iSquadIndex = 0;

#define RGRUNT_GUN_SPREAD 0.08716f
...
#define RGRUNT_LIMP_HEALTH				20
#define RGRUNT_DMG_HEADSHOT				( DMG_BULLET | DMG_CLUB )	// damage types that can kill a grunt with a single headshot.
#define RGRUNT_NUM_HEADS				2 // how many grunt heads are there? 
#define RGRUNT_MINIMUM_HEADSHOT_DAMAGE	15 // must do at least this much damage in one shot to head to score a headshot kill
#define	RGRUNT_SENTENCE_VOLUME			(float)0.35 // volume of grunt sentences
...
//=========================================================
#define		RGRUNT_AE_RELOAD		( 2 )
#define		RGRUNT_AE_KICK			( 3 )
#define		RGRUNT_AE_BURST1		( 4 )
#define		RGRUNT_AE_BURST2		( 5 ) 
#define		RGRUNT_AE_BURST3		( 6 ) 
#define		RGRUNT_AE_GREN_TOSS		( 7 )
#define		RGRUNT_AE_GREN_LAUNCH	( 8 )
#define		RGRUNT_AE_GREN_DROP		( 9 )
#define		RGRUNT_AE_CAUGHT_ENEMY	( 10) // grunt established sight with an enemy (player only) that had previously eluded the squad.
#define		RGRUNT_AE_DROP_GUN		( 11) // grunt (probably dead) is dropping his mp5.


const char *CNPC_RGrunt::pGruntSentences[] =
{
...
enum
{
	...
	RGRUNT_SENT_CHARGE,
	RGRUNT_SENT_TAUNT,
} RGRUNT_SENTENCE_TYPES;

LINK_ENTITY_TO_CLASS(monster_robo_grunt, CNPC_RGrunt);
...
int ACT_RGRUNT_LAUNCH_GRENADE;
int ACT_RGRUNT_TOSS_GRENADE;
int ACT_RGRUNT_MP5_STANDING;
int ACT_RGRUNT_MP5_CROUCHING;
int ACT_RGRUNT_SHOTGUN_STANDING;
int ACT_RGRUNT_SHOTGUN_CROUCHING;

//---------------------------------------------------------
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC(CNPC_RGrunt)
...
// Spawn
//=========================================================
void CNPC_RGrunt::Spawn()
{
	Precache();

	SetModel("models/robogrunt/rgrunt.mdl");
	...
	if (m_iWeapons == 0)
	{
		...
		m_iWeapons = RGRUNT_9MMAR | RGRUNT_HANDGRENADE;
		// pev->weapons = RGRUNT_SHOTGUN;
		// pev->weapons = RGRUNT_9MMAR | RGRUNT_GRENADELAUNCHER;
	}

	if (FBitSet(m_iWeapons, RGRUNT_SHOTGUN))
	{
	...
	else if (FBitSet(m_iWeapons, RGRUNT_GRENADELAUNCHER))
	{
...
int CNPC_RGrunt::IRelationPriority(CBaseEntity *pTarget)
{
...
//=========================================================
void CNPC_RGrunt::Precache()
{
	m_iAmmoType = GetAmmoDef()->Index("SMG1");

	PrecacheModel("models/robogrunt/rgrunt.mdl");
...
//=========================================================
bool CNPC_RGrunt::FOkToSpeak(void)
{
...
// started moving.
//=========================================================
void CNPC_RGrunt::SpeakSentence(void)
{
	if (m_iSentence == RGRUNT_SENT_NONE)
	{
		// no sentence cued up.
		return;
	}
	...
	if (FOkToSpeak())
	{
		SENTENCEG_PlayRndSz(edict(), pGruntSentences[m_iSentence], RGRUNT_SENTENCE_VOLUME, GRUNT_SNDLVL, 0, m_voicePitch);
		JustSpoke();
	}
}
...
//=========================================================
//=========================================================
void CNPC_RGrunt::JustSpoke(void)
{
...
//=========================================================
void CNPC_RGrunt::PrescheduleThink(void)
{
	...
	if (m_pSquad && GetEnemy() != NULL)
	{
		...
		CNPC_RGrunt *pSquadLeader = (CNPC_RGrunt*)m_pSquad->GetLeader()->MyNPCPointer();
...
Class_T	CNPC_RGrunt::Classify(void)
{
...
//
//=========================================================
int CNPC_RGrunt::SquadRecruit(int searchRadius, int maxMembers)
{
	int squadCount;
	...
	if (m_SquadName != NULL_STRING)
	{
		...
		pEntity = gEntList.FindEntityByClassname(pEntity, "monster_robo_grunt");

		while (pEntity)
		{
			CNPC_RGrunt *pRecruit = (CNPC_RGrunt*)pEntity->MyNPCPointer();
	...
	else
	{
		...
		{
			if (!FClassnameIs(pEntity, "monster_robo_grunt"))
				continue;
...
void CNPC_RGrunt::StartNPC(void)
{
	if (!m_pSquad)
	{
		if (m_SquadName != NULL_STRING)
...
// CheckMeleeAttack1
//=========================================================
int CNPC_RGrunt::MeleeAttack1Conditions(float flDot, float flDist)
{
	if (flDist > 64)
...
//=========================================================
// CheckRangeAttack1 - overridden for RGrunt, cause 
// FCanCheckAttacks() doesn't disqualify all attacks based
// on whether or not the enemy is occluded because unlike
// the base class, the RGrunt can attack when the enemy is
// occluded (throw grenade over wall, etc). We must 
// disqualify the machine gun attack if the enemy is occluded.
//=========================================================
int CNPC_RGrunt::RangeAttack1Conditions(float flDot, float flDist)
{
...
int CNPC_RGrunt::RangeAttack2Conditions(float flDot, float flDist)
{
	m_iLastGrenadeCondition = GetGrenadeConditions(flDot, flDist);
	return m_iLastGrenadeCondition;
}

int CNPC_RGrunt::GetGrenadeConditions(float flDot, float flDist)
{
	if (!FBitSet(m_iWeapons, (RGRUNT_HANDGRENADE | RGRUNT_GRENADELAUNCHER)))
		return COND_NONE;
	...
	if (FBitSet(m_iWeapons, RGRUNT_HANDGRENADE))
	{
		// find feet
		if (random->RandomInt(0, 1))
		{
...
//=========================================================
bool CNPC_RGrunt::FCanCheckAttacks(void)
{
...
int CNPC_RGrunt::GetSoundInterests(void)
{
	return	SOUND_WORLD |
		SOUND_COMBAT |
		SOUND_PLAYER |
...
//=========================================================
void CNPC_RGrunt::TraceAttack(const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr)
{
...
//=========================================================
int CNPC_RGrunt::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo)
{
...
//=========================================================
float CNPC_RGrunt::MaxYawSpeed(void)
{
	float flYS;
...
void CNPC_RGrunt::IdleSound(void)
{
	if (FOkToSpeak() && (g_fRGruntQuestion || random->RandomInt(0, 1)))
	{
		if (!g_fRGruntQuestion)
		{
			// ask question or make statement
			switch (random->RandomInt(0, 2))
			{
			case 0: // check in
				SENTENCEG_PlayRndSz(edict(), "RG_CHECK", RGRUNT_SENTENCE_VOLUME, SNDLVL_NORM, 0, m_voicePitch);
				g_fRGruntQuestion = 1;
				break;
			case 1: // question
				SENTENCEG_PlayRndSz(edict(), "RG_QUEST", RGRUNT_SENTENCE_VOLUME, SNDLVL_NORM, 0, m_voicePitch);
				g_fRGruntQuestion = 2;
				break;
			case 2: // statement
				SENTENCEG_PlayRndSz(edict(), "RG_IDLE", RGRUNT_SENTENCE_VOLUME, SNDLVL_NORM, 0, m_voicePitch);
				break;
			}
		}
		else
		{
			switch (g_fRGruntQuestion)
			{
			case 1: // check in
				SENTENCEG_PlayRndSz(edict(), "RG_CLEAR", RGRUNT_SENTENCE_VOLUME, SNDLVL_NORM, 0, m_voicePitch);
				break;
			case 2: // question 
				SENTENCEG_PlayRndSz(edict(), "RG_ANSWER", RGRUNT_SENTENCE_VOLUME, SNDLVL_NORM, 0, m_voicePitch);
				break;
			}
			g_fRGruntQuestion = 0;
		}
...
bool CNPC_RGrunt::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_RGrunt::CheckAmmo(void)
{
	if (m_cAmmoLoaded <= 0)
...
//=========================================================
//=========================================================
CBaseEntity *CNPC_RGrunt::Kick(void)
{
	trace_t tr;
...
Vector CNPC_RGrunt::Weapon_ShootPosition(void)
{
	if (m_fStanding)
...
void CNPC_RGrunt::Event_Killed(const CTakeDamageInfo &info)
{
	Vector	vecGunPos;
	QAngle	vecGunAngles;
	...
	if (FBitSet(m_iWeapons, RGRUNT_GRENADELAUNCHER))
	{
		DropItem("ammo_ARgrenades", BodyTarget(GetAbsOrigin()), vecGunAngles);
	}
	Dissolve("", gpGlobals->curtime, false, ENTITY_DISSOLVE_NORMAL);
...
//=========================================================
void CNPC_RGrunt::HandleAnimEvent(animevent_t *pEvent)
{
	Vector	vecShootDir;
	...
	case RGRUNT_AE_BURST1:
	{
		if (FBitSet(m_iWeapons, RGRUNT_9MMAR))
		{
			Shoot();
	...
	case RGRUNT_AE_BURST2:
	case RGRUNT_AE_BURST3:
		Shoot();
		break;
	...
	case RGRUNT_AE_CAUGHT_ENEMY:
	{
		if (FOkToSpeak())
		{
			SENTENCEG_PlayRndSz(edict(), "RG_ALERT", RGRUNT_SENTENCE_VOLUME, GRUNT_SNDLVL, 0, m_voicePitch);
			JustSpoke();
		}
...
void CNPC_RGrunt::SetAim(const Vector &aimDir)
{
	QAngle angDir;
...
// start task
//=========================================================
void CNPC_RGrunt::StartTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
...
// RunTask
//=========================================================
void CNPC_RGrunt::RunTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_GRUNT_FACE_TOSS_DIR:
...
// PainSound
//=========================================================
void CNPC_RGrunt::PainSound(const CTakeDamageInfo &info)
{
...
// DeathSound 
//=========================================================
void CNPC_RGrunt::DeathSound(const CTakeDamageInfo &info)
{
...
// SetActivity 
//=========================================================
Activity CNPC_RGrunt::NPC_TranslateActivity(Activity eNewActivity)
{
	switch (eNewActivity)
	{
	case ACT_RANGE_ATTACK1:
		...
		if (FBitSet(m_iWeapons, RGRUNT_9MMAR))
		{
			if (m_fStanding)
			{
				...
				return (Activity)ACT_RGRUNT_MP5_STANDING;
			}
			else
			{
				// get crouching shoot
				return (Activity)ACT_RGRUNT_MP5_CROUCHING;
			}
		}
		else
		{
			...
			{
				// get aimable sequence
				return (Activity)ACT_RGRUNT_SHOTGUN_STANDING;
			}
			else
			{
				// get crouching shoot
				return (Activity)ACT_RGRUNT_SHOTGUN_CROUCHING;
			}
		}
		break;
	case ACT_RANGE_ATTACK2:
		...
		if (m_iWeapons & RGRUNT_HANDGRENADE)
		{
			// get toss anim
			return (Activity)ACT_RGRUNT_TOSS_GRENADE;
		}
		else
		{
			// get launch anim
			return (Activity)ACT_RGRUNT_LAUNCH_GRENADE;
		}
		break;
	case ACT_RUN:
		if (m_iHealth <= RGRUNT_LIMP_HEALTH)
		{
			// limp!
			return ACT_RUN_HURT;
		}
...
void CNPC_RGrunt::ClearAttackConditions(void)
{
...
int CNPC_RGrunt::SelectSchedule(void)
{
	// clear old sentence
	m_iSentence = RGRUNT_SENT_NONE;
	...
	if (HasCondition(COND_HEAR_DANGER))
	{
		...
		if (FOkToSpeak())
		{
			SENTENCEG_PlayRndSz(edict(), "RG_GREN", RGRUNT_SENTENCE_VOLUME, GRUNT_SNDLVL, 0, m_voicePitch);
			JustSpoke();
		}
	...
	case NPC_STATE_COMBAT:
	{
				...
				else
				{
					...
					if (FOkToSpeak())// && RANDOM_LONG(0,1))
					{
						if ((GetEnemy() != NULL) && GetEnemy()->IsPlayer())
							...
							(GetEnemy()->Classify() != CLASS_MACHINE_HL1))
							// monster
							SENTENCEG_PlayRndSz(edict(), "RG_MONST", RGRUNT_SENTENCE_VOLUME, GRUNT_SNDLVL, 0, m_voicePitch);
		...
		else if (HasCondition(COND_LIGHT_DAMAGE))
		{
			...
			if (iPercent <= 90 && GetEnemy() != NULL)
			{
				...
				if (FOkToSpeak()) // && RANDOM_LONG(0,1))
				{
					//SENTENCEG_PlayRndSz( ENT(pev), "RG_COVER", RGRUNT_SENTENCE_VOLUME, GRUNT_SNDLVL, 0, m_voicePitch);
					m_iSentence = RGRUNT_SENT_COVER;
					//JustSpoke();
				}
				return SCHED_TAKE_COVER_FROM_ENEMY;
			}
		...
		else if (FBitSet(m_iWeapons, RGRUNT_GRENADELAUNCHER) && HasCondition(COND_CAN_RANGE_ATTACK2) && OccupyStrategySlotRange(SQUAD_SLOT_GRENADE1, SQUAD_SLOT_GRENADE2))
		{
		...
		else if (HasCondition(COND_ENEMY_OCCLUDED))
		{
			if (HasCondition(COND_CAN_RANGE_ATTACK2) && OccupyStrategySlotRange(SQUAD_SLOT_GRENADE1, SQUAD_SLOT_GRENADE2))
			{
				...
				if (FOkToSpeak())
				{
					SENTENCEG_PlayRndSz(edict(), "RG_THROW", RGRUNT_SENTENCE_VOLUME, GRUNT_SNDLVL, 0, m_voicePitch);
					JustSpoke();
				}
				return SCHED_RANGE_ATTACK2;
			}
			else if (OccupyStrategySlotRange(SQUAD_SLOT_ENGAGE1, SQUAD_SLOT_ENGAGE2))
			{
				...
				if (FOkToSpeak())// && RANDOM_LONG(0,1))
				{
					//SENTENCEG_PlayRndSz( ENT(pev), "RG_CHARGE", RGRUNT_SENTENCE_VOLUME, GRUNT_SNDLVL, 0, m_voicePitch);
					m_iSentence = RGRUNT_SENT_CHARGE;
					//JustSpoke();
				}
			...
			else
			{
				...
				{
					SENTENCEG_PlayRndSz(edict(), "RG_TAUNT", RGRUNT_SENTENCE_VOLUME, GRUNT_SNDLVL, 0, m_voicePitch);
					JustSpoke();
				}
...
int CNPC_RGrunt::TranslateSchedule(int scheduleType)
{
...
//=========================================================
// CRGruntRepel - when triggered, spawns a monster_robo_grunt
// repelling down a line.
//=========================================================

class CNPC_RGruntRepel :public CAI_BaseNPC
{
	DECLARE_CLASS(CNPC_RGruntRepel, CAI_BaseNPC);
public:
	void Spawn(void);
	void Precache(void);
...
LINK_ENTITY_TO_CLASS(monster_rgrunt_repel, CNPC_RGruntRepel);
...
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC(CNPC_RGruntRepel)
DEFINE_USEFUNC(RepelUse),
//DEFINE_FIELD( m_iSpriteTexture, FIELD_INTEGER ),
END_DATADESC()

void CNPC_RGruntRepel::Spawn(void)
{
	Precache();
	SetSolid(SOLID_NONE);

	SetUse(&CNPC_RGruntRepel::RepelUse);
}

void CNPC_RGruntRepel::Precache(void)
{
	UTIL_PrecacheOther("monster_robo_grunt");
	m_iSpriteTexture = PrecacheModel("sprites/rope.vmt");
}

void CNPC_RGruntRepel::RepelUse(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	trace_t tr;
	UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, -4096.0), MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr);

	CBaseEntity *pEntity = Create("monster_robo_grunt", GetAbsOrigin(), GetAbsAngles());
...
//
//------------------------------------------------------------------------------
AI_BEGIN_CUSTOM_NPC(monster_robo_grunt, CNPC_RGrunt)

DECLARE_ACTIVITY(ACT_RGRUNT_LAUNCH_GRENADE)
DECLARE_ACTIVITY(ACT_RGRUNT_TOSS_GRENADE)
DECLARE_ACTIVITY(ACT_RGRUNT_MP5_STANDING);
DECLARE_ACTIVITY(ACT_RGRUNT_MP5_CROUCHING);
DECLARE_ACTIVITY(ACT_RGRUNT_SHOTGUN_STANDING);
DECLARE_ACTIVITY(ACT_RGRUNT_SHOTGUN_CROUCHING);
...
// DEAD RGRUNT PROP
//=========================================================
class CNPC_DeadRGrunt : public CAI_BaseNPC
{
	DECLARE_CLASS(CNPC_DeadRGrunt, CAI_BaseNPC);
public:
	void Spawn(void);
...
char *CNPC_DeadRGrunt::m_szPoses[] = { "deadstomach", "deadside", "deadsitting" };

bool CNPC_DeadRGrunt::KeyValue(const char *szKeyName, const char *szValue)
{
...
LINK_ENTITY_TO_CLASS(monster_rgrunt_dead, CNPC_DeadRGrunt);

//=========================================================
// ********** DeadRGrunt SPAWN **********
//=========================================================
void CNPC_DeadRGrunt::Spawn(void)
{
	PrecacheModel("models/robogrunt/rgrunt.mdl");
	SetModel("models/robogrunt/rgrunt.mdl");
