...
CBasePlayer *UTIL_GetListenServerHost( void )
{
	...
	{
#ifndef TF_CLASSIC //This cause a lot of spam in dedicated servers
...
void UTIL_ScreenShakeObject( CBaseEntity *pEnt, const Vector &center, float amplitude, float frequency, float duration, float radius, ShakeCommand_t eCommand, bool bAirShake )
{
	...
	for ( i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		else if (pPlayer && (pPlayer->GetFlags() & FL_ONGROUND) && (pPlayer->GetGroundEntity()->GetRootMoveParent() == pHighestParent))
		{
