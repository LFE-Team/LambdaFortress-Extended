...
int g_fCombineSynthQuestion;				// true if an idle grunt asked a question. Cleared when someone answers. YUCK old global from grunt code

#define COMBINE_SYNTH_SKIN_DEFAULT		0
#define COMBINE_SYNTH_SKIN_SHOTGUNNER		1


#define COMBINE_SYNTH_GRENADE_THROW_SPEED 650
#define COMBINE_SYNTH_GRENADE_TIMER		3.5
#define COMBINE_SYNTH_GRENADE_FLUSH_TIME	3.0		// Don't try to flush an enemy who has been out of sight for longer than this.
#define COMBINE_SYNTH_GRENADE_FLUSH_DIST	256.0	// Don't try to flush an enemy who has moved farther than this distance from the last place I saw him.

#define COMBINE_SYNTH_LIMP_HEALTH				20
#define	COMBINE_SYNTH_MIN_GRENADE_CLEAR_DIST	250

#define COMBINE_SYNTH_EYE_STANDING_POSITION	Vector( 0, 0, 66 )
#define COMBINE_SYNTH_GUN_STANDING_POSITION	Vector( 0, 0, 57 )
#define COMBINE_SYNTH_EYE_CROUCHING_POSITION	Vector( 0, 0, 40 )
#define COMBINE_SYNTH_GUN_CROUCHING_POSITION	Vector( 0, 0, 36 )
#define COMBINE_SYNTH_SHOTGUN_STANDING_POSITION	Vector( 0, 0, 36 )
#define COMBINE_SYNTH_SHOTGUN_CROUCHING_POSITION	Vector( 0, 0, 36 )
#define COMBINE_SYNTH_MIN_CROUCH_DISTANCE		256.0
...
string_t	s_iszShotgunSynthClassname;
...
//=========================================================
#define COMBINE_SYNTH_AE_RELOAD			( 2 )
#define COMBINE_SYNTH_AE_KICK				( 3 )
#define COMBINE_SYNTH_AE_AIM				( 4 )
#define COMBINE_SYNTH_AE_GREN_TOSS		( 7 )
#define COMBINE_SYNTH_AE_GREN_LAUNCH		( 8 )
#define COMBINE_SYNTH_AE_GREN_DROP		( 9 )
#define COMBINE_SYNTH_AE_CAUGHT_ENEMY		( 10) // grunt established sight with an enemy (player only) that had previously eluded the squad.

int COMBINE_SYNTH_AE_BEGIN_ALTFIRE;
int COMBINE_SYNTH_AE_ALTFIRE;

//=========================================================
// Combine activities
//=========================================================
//Activity ACT_COMBINE_SYNTH_STANDING_SMG1;
//Activity ACT_COMBINE_SYNTH_CROUCHING_SMG1;
//Activity ACT_COMBINE_SYNTH_STANDING_AR2;
//Activity ACT_COMBINE_SYNTH_CROUCHING_AR2;
//Activity ACT_COMBINE_SYNTH_WALKING_AR2;
//Activity ACT_COMBINE_SYNTH_STANDING_SHOTGUN;
//Activity ACT_COMBINE_SYNTH_CROUCHING_SHOTGUN;
Activity ACT_COMBINE_SYNTH_THROW_GRENADE;
Activity ACT_COMBINE_SYNTH_LAUNCH_GRENADE;
Activity ACT_COMBINE_SYNTH_BUGBAIT;
Activity ACT_COMBINE_SYNTH_AR2_ALTFIRE;
Activity ACT_SYNTH_WALK_EASY;
Activity ACT_SYNTH_WALK_MARCH;
...
LINK_ENTITY_TO_CLASS(npc_combine_synth_base, CNPC_CombineSynth);

//---------------------------------------------------------
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CNPC_CombineSynth )
...
// Constructor.
//------------------------------------------------------------------------------
CNPC_CombineSynth::CNPC_CombineSynth()
{
...
// Create components
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::CreateComponents()
{
...
//------------------------------------------------------------------------------
void CNPC_CombineSynth::InputLookOff( inputdata_t &inputdata )
{
	m_spawnflags |= SF_COMBINE_SYNTH_NO_LOOK;
}

//------------------------------------------------------------------------------
// Purpose: Enable looking.
//------------------------------------------------------------------------------
void CNPC_CombineSynth::InputLookOn( inputdata_t &inputdata )
{
	m_spawnflags &= ~SF_COMBINE_SYNTH_NO_LOOK;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::InputStartPatrolling( inputdata_t &inputdata )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::InputStopPatrolling( inputdata_t &inputdata )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::InputAssault( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::InputHitByBugbait( inputdata_t &inputdata )
{
	SetCondition( COND_COMBINE_SYNTH_HIT_BY_BUGBAIT );
}
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::InputThrowGrenadeAtTarget( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::Activate()
{
	s_iszShotgunSynthClassname = FindPooledString( "weapon_shotgun" );
	BaseClass::Activate();
}
...
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::CreateBehaviors()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::PostNPCInit()
{
	if( IsElite() )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::GatherConditions()
{
	BaseClass::GatherConditions();

	ClearCondition( COND_COMBINE_SYNTH_ATTACK_SLOT_AVAILABLE );

	if( GetState() == NPC_STATE_COMBAT )
	{
		if( IsCurSchedule( SCHED_COMBINE_SYNTH_WAIT_IN_COVER, false ) )
		{
			...
			{
				SetCondition( COND_COMBINE_SYNTH_ATTACK_SLOT_AVAILABLE );
			}
		}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::PrescheduleThink()
{
	...
	if ( IsOnFire() )
	{
		SetCondition( COND_COMBINE_SYNTH_ON_FIRE );
	}
	else
	{
		ClearCondition( COND_COMBINE_SYNTH_ON_FIRE );
	}
#endif
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::DelayAltFireAttack( float flDelay )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::DelaySquadAltFireAttack( float flDelay )
{
	...
	while ( pSquadmate )
	{
		CNPC_CombineSynth *pCombine = dynamic_cast<CNPC_CombineSynth*>(pSquadmate);
...
//-----------------------------------------------------------------------------
float CNPC_CombineSynth::MaxYawSpeed( void )
{
	switch( GetActivity() )
	{
	case ACT_TURN_LEFT:
...
// 
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::ShouldMoveAndShoot()
{
	...
	if( IsCurSchedule( SCHED_COMBINE_SYNTH_HIDE_AND_RELOAD, false ) )
	...
	if( IsCurSchedule( SCHED_COMBINE_SYNTH_TAKE_COVER_FROM_BEST_SOUND, false ) )
		return false;

	if( IsCurSchedule( SCHED_COMBINE_SYNTH_RUN_AWAY_FROM_BEST_SOUND, false ) )
		return false;

	if( HasCondition( COND_NO_PRIMARY_AMMO, false ) )
		m_flStopMoveShootTime = gpGlobals->curtime + random->RandomFloat( 0.4f, 0.6f );

	if( m_pSquad && IsCurSchedule( SCHED_COMBINE_SYNTH_TAKE_COVER1, false ) )
		m_flStopMoveShootTime = gpGlobals->curtime + random->RandomFloat( 0.4f, 0.6f );
...
// Output :
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::OverrideMoveFacing( const AILocalMoveGoal_t &move, float flInterval )
{
...
//
//-----------------------------------------------------------------------------
Class_T	CNPC_CombineSynth::Classify ( void )
{
...
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::IsCurTaskContinuousMove()
{
	const Task_t* pTask = GetTask();
	if ( pTask && (pTask->iTask == TASK_COMBINE_SYNTH_CHASE_ENEMY_CONTINUOUSLY) )
		return true;
...
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::StartTaskChaseEnemyContinuously( const Task_t *pTask )
{
...
void CNPC_CombineSynth::RunTaskChaseEnemyContinuously( const Task_t *pTask )
{
	if (!GetNavigator()->IsGoalActive())
	{
		SetIdealActivity( GetStoppedActivity() );
...
// start task
//=========================================================
void CNPC_CombineSynth::StartTask( const Task_t *pTask )
{
	// NOTE: This reset is required because we change it in TASK_COMBINE_SYNTH_CHASE_ENEMY_CONTINUOUSLY
	m_MoveAndShootOverlay.SetInitialDelay( 0.75 );

	switch ( pTask->iTask )
	{
	case TASK_COMBINE_SYNTH_SET_STANDING:
		{
	...
	case TASK_ANNOUNCE_ATTACK:
		{
			...
			if ((int)pTask->flTaskData == 1)
			{
					...
					(gpGlobals->curtime - m_flLastAttackTime > 3.0) )
				{
					...
					m_Sentences.Speak( "COMBINE_SYNTH_ANNOUNCE", SENTENCE_PRIORITY_HIGH );
			...
			{
				m_Sentences.Speak( "COMBINE_SYNTH_THROW_GRENADE", SENTENCE_PRIORITY_MEDIUM );
				SetActivity(ACT_IDLE);
	...
	case TASK_COMBINE_SYNTH_FACE_TOSS_DIR:
		break;

	case TASK_COMBINE_SYNTH_GET_PATH_TO_FORCED_GREN_LOS:
		{
	...
	case TASK_COMBINE_SYNTH_IGNORE_ATTACKS:
		// must be in a squad
	...
	case TASK_COMBINE_SYNTH_DEFER_SQUAD_GRENADES:
		{
			if ( m_pSquad )
			{
			...
			if (GetHintGroup() == NULL_STRING)
			{
				...
				if ( pEntity )
				{
					...
					if ( !HasMemory(bits_MEMORY_PLAYER_HURT) && pEntity->IsPlayer() && pEntity->GetHealth() <= 20 )
					{
						...
						m_Sentences.Speak( "COMBINE_SYNTH_PLAYERHIT", SENTENCE_PRIORITY_INVALID );
	...
	case TASK_COMBINE_SYNTH_DIE_INSTANTLY:
		{
...
// RunTask
//=========================================================
void CNPC_CombineSynth::RunTask( const Task_t *pTask )
{
	/*
	{
	CBaseEntity *pEnemy = GetEnemy();
...
// Output  :
//------------------------------------------------------------------------------
Vector CNPC_CombineSynth::BodyTarget( const Vector &posSrc, bool bNoisy ) 
{
...
// Purpose:
//------------------------------------------------------------------------------
bool CNPC_CombineSynth::FVisible( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
	if( m_spawnflags & SF_COMBINE_SYNTH_NO_LOOK )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::Event_Killed( const CTakeDamageInfo &info )
{
	...
	{
		if( m_iLastAnimEventHandled != COMBINE_SYNTH_AE_GREN_TOSS )
		{
...
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::UpdateEnemyMemory( CBaseEntity *pEnemy, const Vector &position, CBaseEntity *pInformer )
{
	if( m_spawnflags & SF_COMBINE_SYNTH_NO_LOOK )
	{
		return false;
...
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::BuildScheduleTestBits( void )
{
	...
	SetCustomInterruptCondition( COND_COMBINE_SYNTH_HIT_BY_BUGBAIT );

	if ( !IsCurSchedule( SCHED_COMBINE_SYNTH_BURNING_STAND ) )
	{
		SetCustomInterruptCondition( COND_COMBINE_SYNTH_ON_FIRE );
	}
}
...
//-----------------------------------------------------------------------------
Activity CNPC_CombineSynth::NPC_TranslateActivity( Activity eNewActivity )
{
	//Slaming this back to ACT_COMBINE_SYNTH_BUGBAIT since we don't want ANYTHING to change our activity while we burn.
	if ( HasCondition( COND_COMBINE_SYNTH_ON_FIRE ) )
		return BaseClass::NPC_TranslateActivity( ACT_COMBINE_SYNTH_BUGBAIT );
	...
	if (eNewActivity == ACT_RANGE_ATTACK2)
	{
		...
		{
			return ( Activity )ACT_COMBINE_SYNTH_LAUNCH_GRENADE;
		}
		else
		{
			return ( Activity )ACT_COMBINE_SYNTH_THROW_GRENADE;
		}
	}
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_CombineSynth::GetSoundInterests( void )
{
...
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::QueryHearSound( CSound *pSound )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::AnnounceAssault(void)
{
	...
	if ( FVisible( pBCC ) )
	{
		m_Sentences.Speak( "COMBINE_SYNTH_ASSAULT" );
	}
}


void CNPC_CombineSynth::AnnounceEnemyType( CBaseEntity *pEnemy )
{
	const char *pSentenceName = "COMBINE_SYNTH_MONST";
	switch ( pEnemy->Classify() )
	{
	case CLASS_PLAYER:
		pSentenceName = "COMBINE_SYNTH_ALERT";
		break;
	...
	case CLASS_VORTIGAUNT:
		pSentenceName = "COMBINE_SYNTH_MONST_CITIZENS";
		break;

	case CLASS_PLAYER_ALLY_VITAL:
		pSentenceName = "COMBINE_SYNTH_MONST_CHARACTER";
		break;

	case CLASS_ANTLION:
		pSentenceName = "COMBINE_SYNTH_MONST_BUGS";
		break;

	case CLASS_ZOMBIE:
		pSentenceName = "COMBINE_SYNTH_MONST_ZOMBIES";
		break;

	case CLASS_HEADCRAB:
	case CLASS_BARNACLE:
		pSentenceName = "COMBINE_SYNTH_MONST_PARASITES";
		break;
	}
...
void CNPC_CombineSynth::AnnounceEnemyKill( CBaseEntity *pEnemy )
{
	if (!pEnemy )
		return;

	const char *pSentenceName = "COMBINE_SYNTH_KILL_MONST";
	switch ( pEnemy->Classify() )
	{
	case CLASS_PLAYER:
		pSentenceName = "COMBINE_SYNTH_PLAYER_DEAD";
		break;
...
//-----------------------------------------------------------------------------
int CNPC_CombineSynth::SelectCombatSchedule()
{
	// -----------
	// dead enemy
	// -----------
	...
	if ( HasCondition( COND_NEW_ENEMY ) )
	{
		...
		if ( m_pSquad && pEnemy )
		{
			...
			if ( HasCondition( COND_CAN_RANGE_ATTACK1 ) && OccupyStrategySlot( SQUAD_SLOT_ATTACK1 ) )
			{
				...
				return SCHED_COMBINE_SYNTH_SUPPRESS;
			}
			...
			if ( m_pSquad->IsLeader( this ) || ( m_pSquad->GetLeader() && m_pSquad->GetLeader()->GetEnemy() != pEnemy ) )
			{
				...
				// This code only runs if the code above didn't assign me SCHED_COMBINE_SYNTH_SUPPRESS.
				...
				if( HasCondition(COND_WEAPON_HAS_LOS) && IsStrategySlotRangeOccupied( SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK2 ) )
				{
					...
					{
						return SCHED_COMBINE_SYNTH_ENTER_OVERWATCH;
					}
				}
			}
			else
			{
				...
				if( !bFirstContact && OccupyStrategySlotRange( SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK2 ) )
				{
					...
					{
						return SCHED_COMBINE_SYNTH_PRESS_ATTACK;
					}
				}
		...
		if ( GetEnemy() != NULL )
		{
			...
			if( !IsCrouching() )
			{
				...
				else
				{
					...
					// m_Sentences.Speak( "COMBINE_SYNTH_COVER" );
					return SCHED_TAKE_COVER_FROM_ENEMY;
				}
			}
		}
	...
	if (HasCondition(COND_ENEMY_OCCLUDED))
	{
		...
		return SCHED_COMBINE_SYNTH_WAIT_IN_COVER;
	}
	...
	if ( HasCondition( COND_SEE_ENEMY ) && !HasCondition( COND_CAN_RANGE_ATTACK1 ) )
	{
		...
		AnnounceAssault(); 
		return SCHED_COMBINE_SYNTH_ASSAULT;
	}
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_CombineSynth::SelectSchedule( void )
{
...
#ifndef TF_CLASSIC
	if ( HasCondition(COND_COMBINE_SYNTH_ON_FIRE) )
		return SCHED_COMBINE_SYNTH_BURNING_STAND;
#endif
	...
	if ( m_hForcedGrenadeTarget )
	{
		if ( m_flNextGrenadeCheck < gpGlobals->curtime )
		{
			...
			if ( IsElite() )
			{
				if ( FVisible( m_hForcedGrenadeTarget ) )
				{
					...
					return SCHED_COMBINE_SYNTH_AR2_ALTFIRE;
				}
			}
			else
			{
				...
				{
					m_hForcedGrenadeTarget = NULL;
					return SCHED_COMBINE_SYNTH_FORCED_GRENADE_THROW;
				}
			}
		}
		...
		if ( !FVisible( m_hForcedGrenadeTarget ) )
		{
			return SCHED_COMBINE_SYNTH_MOVE_TO_FORCED_GREN_LOS;
		}
	}
	...
	if ( m_NPCState != NPC_STATE_SCRIPT)
	{
		...
		if ( HasCondition( COND_COMBINE_SYNTH_HIT_BY_BUGBAIT ) )
		{
			...
			ClearCondition( COND_COMBINE_SYNTH_HIT_BY_BUGBAIT );
			return SCHED_COMBINE_SYNTH_BUGBAIT_DISTRACTION;
		}
			...
			if ( HasCondition(COND_HEAR_DANGER) )
			{
				...
				if ( pSound)
				{
					if (pSound->m_iType & SOUND_DANGER)
					{
						...
						const char *pSentenceName = "COMBINE_SYNTH_DANGER";
						...
						if ( pSoundOwner )
						{
							...
							if ( pGrenade && pGrenade->GetThrower() )
							{
								if ( IRelationType( pGrenade->GetThrower() ) != D_LI )
								{
									...
									pSentenceName = "COMBINE_SYNTH_GREN";
								}
							}
						}
						...
						if( !GetEnemy() && pSound->IsSoundType(SOUND_CONTEXT_DANGER_APPROACH) && pSound->m_hOwner && !FInViewCone(pSound->GetSoundReactOrigin()) )
						{
							...
							return SCHED_COMBINE_SYNTH_FACE_IDEAL_YAW;
						}
	...
	case NPC_STATE_IDLE:
		{
			if ( m_bShouldPatrol )
				return SCHED_COMBINE_SYNTH_PATROL;
		}
		// NOTE: Fall through!
	...
	case NPC_STATE_ALERT:
		{
			...
			{
				if( m_bShouldPatrol || HasCondition( COND_COMBINE_SYNTH_SHOULD_PATROL ) )
					return SCHED_COMBINE_SYNTH_PATROL;
			}
		}
		break;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_CombineSynth::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
	if( failedSchedule == SCHED_COMBINE_SYNTH_TAKE_COVER1 )
	{
		if( IsInSquad() && IsStrategySlotRangeOccupied(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK2) && HasCondition(COND_SEE_ENEMY) )
		{
			...
			return SCHED_COMBINE_SYNTH_MOVE_TO_MELEE;
		}
	}
...
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::ShouldChargePlayer()
{
...
//-----------------------------------------------------------------------------
#define COMBINE_SYNTH_MEGA_PHYSCANNON_ATTACK_DISTANCE	192
#define COMBINE_SYNTH_MEGA_PHYSCANNON_ATTACK_DISTANCE_SQ	(COMBINE_SYNTH_MEGA_PHYSCANNON_ATTACK_DISTANCE*COMBINE_SYNTH_MEGA_PHYSCANNON_ATTACK_DISTANCE)

int CNPC_CombineSynth::SelectScheduleAttack()
{
	// Drop a grenade?
	if ( HasCondition( COND_COMBINE_SYNTH_DROP_GRENADE ) )
		return SCHED_COMBINE_SYNTH_DROP_GRENADE;
	...
	if ( GetEnemy() && GetEnemy()->Classify() == CLASS_COMBINE && FClassnameIs(GetEnemy(), "npc_turret_floor") )
	{
		...
		if ( !GetEnemy()->MyNPCPointer()->FInViewCone( this ) && OccupyStrategySlot( SQUAD_SLOT_GRENADE1 ) )
			return SCHED_COMBINE_SYNTH_CHARGE_TURRET;
	}
	...
	{
		float flDistSq = GetEnemy()->WorldSpaceCenter().DistToSqr( WorldSpaceCenter() );
		if ( flDistSq <= COMBINE_SYNTH_MEGA_PHYSCANNON_ATTACK_DISTANCE_SQ )
		{
			if( HasCondition(COND_SEE_ENEMY) )
			{
				if ( OccupyStrategySlotRange( SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK2 ) )
		...
		{
			return SCHED_COMBINE_SYNTH_CHARGE_PLAYER;
		}
	}
	...
	if ( GetEnemy() && !HasCondition(COND_SEE_ENEMY) )
	{
		...
		//Msg("Time: %f   Dist: %f\n", flTime, flDist );
		if ( flTime <= COMBINE_SYNTH_GRENADE_FLUSH_TIME && flDist <= COMBINE_SYNTH_GRENADE_FLUSH_DIST && CanGrenadeEnemy( false ) && OccupyStrategySlot( SQUAD_SLOT_GRENADE1 ) )
		{
			return SCHED_RANGE_ATTACK2;
		}
	}
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_CombineSynth::TranslateSchedule( int scheduleType ) 
{
	switch( scheduleType )
	{
	case SCHED_TAKE_COVER_FROM_ENEMY:
		{
			if ( m_pSquad )
			{
				...
				{
					m_Sentences.Speak( "COMBINE_SYNTH_THROW_GRENADE" );
					return SCHED_COMBINE_SYNTH_TOSS_GRENADE_COVER1;
				}
				else
				{
					...
					return SCHED_COMBINE_SYNTH_TAKE_COVER1;
				}
			}
			else
			{
				...
				{
					return SCHED_COMBINE_SYNTH_GRENADE_COVER1;
				}
				else
				{
					if ( ShouldChargePlayer() && !IsUnreachable( GetEnemy() ) )
	...
	case SCHED_TAKE_COVER_FROM_BEST_SOUND:
		{
			return SCHED_COMBINE_SYNTH_TAKE_COVER_FROM_BEST_SOUND;
		}
		break;
	case SCHED_COMBINE_SYNTH_TAKECOVER_FAILED:
		{
			if ( HasCondition( COND_CAN_RANGE_ATTACK1 ) && OccupyStrategySlotRange( SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK2 ) )
			{
	...
	case SCHED_FAIL_ESTABLISH_LINE_OF_FIRE:
		{
			...
			else if ( !m_AssaultBehavior.HasAssaultCue() )
			{
				...
				return TranslateSchedule( SCHED_COMBINE_SYNTH_PATROL );
			}
		}
		break;
	case SCHED_COMBINE_SYNTH_ASSAULT:
		{
			CBaseEntity *pEntity = GetEnemy();
	...
	case SCHED_ESTABLISH_LINE_OF_FIRE:
		{
			...
			return SCHED_COMBINE_SYNTH_ESTABLISH_LINE_OF_FIRE;
		}
		break;
	case SCHED_HIDE_AND_RELOAD:
		{
			...
			if( CanGrenadeEnemy() && OccupyStrategySlot( SQUAD_SLOT_GRENADE1 ) && random->RandomInt( 0, 100 ) < 20 )
			{
				...
				return SCHED_COMBINE_SYNTH_GRENADE_AND_RELOAD;
			}
			...
			return SCHED_COMBINE_SYNTH_HIDE_AND_RELOAD;
		}
		break;
	case SCHED_RANGE_ATTACK1:
		{
			...
			if ( IsCrouching() || ( CrouchIsDesired() && !HasCondition( COND_HEAVY_DAMAGE ) ) )
			{
				...
				if (GetEnemy() != NULL)
				{
					...
					if (dist > COMBINE_SYNTH_MIN_CROUCH_DISTANCE)
					{
						// try crouching
						Crouch();
			...
			return SCHED_COMBINE_SYNTH_RANGE_ATTACK1;
		}
	case SCHED_RANGE_ATTACK2:
		{
			...
			{
				return SCHED_COMBINE_SYNTH_RANGE_ATTACK2;
			}
		}
		// SCHED_COMBAT_FACE:
		// SCHED_COMBINE_SYNTH_WAIT_FACE_ENEMY:
		// SCHED_COMBINE_SYNTH_SWEEP:
		// SCHED_COMBINE_SYNTH_COVER_AND_RELOAD:
		// SCHED_COMBINE_SYNTH_FOUND_ENEMY:

	case SCHED_VICTORY_DANCE:
		{
			return SCHED_COMBINE_SYNTH_VICTORY_DANCE;
		}
	case SCHED_COMBINE_SYNTH_SUPPRESS:
		{
#define MIN_SIGNAL_DIST	256
			if ( GetEnemy() != NULL && GetEnemy()->IsPlayer() && m_bFirstEncounter )
			{
				...
				if( flDistToEnemy >= MIN_SIGNAL_DIST )
				{
					...
					return SCHED_COMBINE_SYNTH_SIGNAL_SUPPRESS;
				}
			}
	...
	case SCHED_FAIL:
		{
			if ( GetEnemy() != NULL )
			{
				return SCHED_COMBINE_SYNTH_COMBAT_FAIL;
			}
			return SCHED_FAIL;
		}

	case SCHED_COMBINE_SYNTH_PATROL:
		{
			// If I have an enemy, don't go off into random patrol mode.
			if ( GetEnemy() && GetEnemy()->IsAlive() )
				return SCHED_COMBINE_SYNTH_PATROL_ENEMY;
...
//=========================================================
//=========================================================
void CNPC_CombineSynth::OnStartSchedule( int scheduleType ) 
{
}
...
//=========================================================
void CNPC_CombineSynth::HandleAnimEvent( animevent_t *pEvent )
{
	Vector vecShootDir;
	...
	{
		if ( pEvent->event == COMBINE_SYNTH_AE_BEGIN_ALTFIRE )
		{
			EmitSound( "Weapon_CombineGuard.Special1" );
			handledEvent = true;
		}
		else if ( pEvent->event == COMBINE_SYNTH_AE_ALTFIRE )
		{
			if( IsElite() )
			{
	...
	else
	{
		switch( pEvent->event )
		{
		case COMBINE_SYNTH_AE_AIM:	
			{
				handledEvent = true;
				break;
			}
		case COMBINE_SYNTH_AE_RELOAD:
		...
		case COMBINE_SYNTH_AE_GREN_TOSS:
			{
				Vector vecSpin;
				...
				if( m_NPCState == NPC_STATE_SCRIPT )
				{
					...
					Fraggrenade_Create( vecStart, vec3_angle, vecThrow, vecSpin, this, COMBINE_SYNTH_GRENADE_TIMER, true );
				}
				else
				{
					// Use the Velocity that AI gave us.
					Fraggrenade_Create( vecStart, vec3_angle, m_vecTossVelocity, vecSpin, this, COMBINE_SYNTH_GRENADE_TIMER, true );
					m_iNumGrenades--;
				}
		...
		case COMBINE_SYNTH_AE_GREN_LAUNCH:
			{
		...
		case COMBINE_SYNTH_AE_GREN_DROP:
			{
				Vector vecStart;
				GetAttachment( "lefthand", vecStart );

				Fraggrenade_Create( vecStart, vec3_angle, m_vecTossVelocity, vec3_origin, this, COMBINE_SYNTH_GRENADE_TIMER, true );
				m_iNumGrenades--;
			}
			handledEvent = true;
			break;

		case COMBINE_SYNTH_AE_KICK:
			{
				...
				m_Sentences.Speak( "COMBINE_SYNTH_KICK" );
				handledEvent = true;
				break;
			}

		case COMBINE_SYNTH_AE_CAUGHT_ENEMY:
			m_Sentences.Speak( "COMBINE_SYNTH_ALERT" );
			handledEvent = true;
			break;
...
// Output :
//-----------------------------------------------------------------------------
Vector CNPC_CombineSynth::Weapon_ShootPosition( )
{
	...
	if  ( bStanding )
	{
		if( HasShotgun() )
		{
			return GetAbsOrigin() + COMBINE_SYNTH_SHOTGUN_STANDING_POSITION + right * 8;
		}
		else
		{
			return GetAbsOrigin() + COMBINE_SYNTH_GUN_STANDING_POSITION + right * 8;
		}
	}
	else
	{
		if( HasShotgun() )
		{
			return GetAbsOrigin() + COMBINE_SYNTH_SHOTGUN_CROUCHING_POSITION + right * 8;
		}
		else
		{
			return GetAbsOrigin() + COMBINE_SYNTH_GUN_CROUCHING_POSITION + right * 8;
		}
	}
}
...
// started moving.
//=========================================================
void CNPC_CombineSynth::SpeakSentence( int sentenceType )
{
	switch( sentenceType )
	{
	case 0: // assault
	...
	case 1: // Flanking the player
		...
		{
			m_Sentences.Speak( "COMBINE_SYNTH_FLANK" );
		}
		break;
	}
}
...
// PainSound
//=========================================================
void CNPC_CombineSynth::PainSound ( const CTakeDamageInfo &info )
{
	...
	{
		const char *pSentenceName = "COMBINE_SYNTH_PAIN";
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::LostEnemySound( void)
{
	...
	{
		pSentence = "COMBINE_SYNTH_LOST_LONG"; 
	}
	else
	{
		pSentence = "COMBINE_SYNTH_LOST_SHORT";
	}
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::FoundEnemySound( void)
{
	m_Sentences.Speak( "COMBINE_SYNTH_REFIND_ENEMY", SENTENCE_PRIORITY_HIGH );
}
...
void CNPC_CombineSynth::AlertSound( void)
{
	if ( gpGlobals->curtime > m_flNextAlertSoundTime )
	{
		m_Sentences.Speak( "COMBINE_SYNTH_GO_ALERT", SENTENCE_PRIORITY_HIGH );
...
// NotifyDeadFriend
//=========================================================
void CNPC_CombineSynth::NotifyDeadFriend ( CBaseEntity* pFriend )
{
	if ( GetSquad()->NumMembers() < 2 )
	{
		m_Sentences.Speak( "COMBINE_SYNTH_LAST_OF_SQUAD", SENTENCE_PRIORITY_INVALID, SENTENCE_CRITERIA_NORMAL );
		JustMadeSound();
		return;
	}
	...
	{
		m_Sentences.Speak( "COMBINE_SYNTH_MAN_DOWN" );
	}
...
// DeathSound 
//=========================================================
void CNPC_CombineSynth::DeathSound ( void )
{
	// NOTE: The response system deals with this at the moment
	if ( GetFlags() & FL_DISSOLVING )
		return;

	m_Sentences.Speak( "COMBINE_SYNTH_DIE", SENTENCE_PRIORITY_INVALID, SENTENCE_CRITERIA_ALWAYS );
}

//=========================================================
// IdleSound 
//=========================================================
void CNPC_CombineSynth::IdleSound( void )
{
	if (g_fCombineSynthQuestion || random->RandomInt(0,1))
	{
		if (!g_fCombineSynthQuestion)
		{
			// ask question or make statement
			switch (random->RandomInt(0,2))
			{
			case 0: // check in
				if ( m_Sentences.Speak( "COMBINE_SYNTH_CHECK" ) >= 0 )
				{
					g_fCombineSynthQuestion = 1;
				}
				break;

			case 1: // question
				if ( m_Sentences.Speak( "COMBINE_SYNTH_QUEST" ) >= 0 )
				{
					g_fCombineSynthQuestion = 2;
				}
				break;

			case 2: // statement
				m_Sentences.Speak( "COMBINE_SYNTH_IDLE" );
				break;
			}
		}
		else
		{
			switch (g_fCombineSynthQuestion)
			{
			case 1: // check in
				if ( m_Sentences.Speak( "COMBINE_SYNTH_CLEAR" ) >= 0 )
				{
					g_fCombineSynthQuestion = 0;
				}
				break;
			case 2: // question 
				if ( m_Sentences.Speak( "COMBINE_SYNTH_ANSWER" ) >= 0 )
				{
					g_fCombineSynthQuestion = 0;
				}
				break;
...
// Output :
//-----------------------------------------------------------------------------
int	CNPC_CombineSynth::RangeAttack2Conditions( float flDot, float flDist ) 
{
	return COND_NONE;
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::CanThrowGrenade( const Vector &vecTarget )
{
	if( m_iNumGrenades < 1 )
	{
		// Out of grenades!
	...
	if ( m_pSquad )
	{
		if (m_pSquad->SquadMemberInRange( vecTarget, COMBINE_SYNTH_MIN_GRENADE_CLEAR_DIST ))
		{
			// crap, I might blow my own guy up. Don't throw a grenade and don't check again for a while.
			m_flNextGrenadeCheck = gpGlobals->curtime + 1; // one full second.

			// Tell my squad members to clear out so I can get a grenade in
			CSoundEnt::InsertSound( SOUND_MOVE_AWAY | SOUND_CONTEXT_COMBINE_ONLY, vecTarget, COMBINE_SYNTH_MIN_GRENADE_CLEAR_DIST, 0.1 );
			return false;
		}
	}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::CheckCanThrowGrenade( const Vector &vecTarget )
{
	...
	{
		vecToss = VecCheckThrow( this, EyePosition(), vecTarget, COMBINE_SYNTH_GRENADE_THROW_SPEED, 1.0, &vecMins, &vecMaxs );
	}
	else
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::CanAltFireEnemy( bool bUseFreeKnowledge )
{
	if (!IsElite() )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::CanGrenadeEnemy( bool bUseFreeKnowledge )
{
	if( IsElite() )
		return false;
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_CombineSynth::MeleeAttack1Conditions ( float flDot, float flDist )
{
	if (flDist > 64)
	{
...
// Output : Vector
//-----------------------------------------------------------------------------
Vector CNPC_CombineSynth::EyePosition( void ) 
{
	if ( !IsCrouching() )
	{
		return GetAbsOrigin() + COMBINE_SYNTH_EYE_STANDING_POSITION;
	}
	else
	{
		return GetAbsOrigin() + COMBINE_SYNTH_EYE_CROUCHING_POSITION;
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
Vector CNPC_CombineSynth::GetAltFireTarget()
{
	Assert( IsElite() );
...
// Output : Vector
//-----------------------------------------------------------------------------
Vector CNPC_CombineSynth::EyeOffset( Activity nActivity )
{
	if (CapabilitiesGet() & bits_CAP_DUCK)
	{
		if ( IsCrouchedActivity( nActivity ) )
			return COMBINE_SYNTH_EYE_CROUCHING_POSITION;

	}
	// if the hint doesn't tell anything, assume current state
	if ( !IsCrouching() )
	{
		return COMBINE_SYNTH_EYE_STANDING_POSITION;
	}
	else
	{
		return COMBINE_SYNTH_EYE_CROUCHING_POSITION;
...
// Purpose: 
//-----------------------------------------------------------------------------
Vector CNPC_CombineSynth::GetCrouchEyeOffset( void )
{
	return COMBINE_SYNTH_EYE_CROUCHING_POSITION;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::SetActivity( Activity NewActivity )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
NPC_STATE CNPC_CombineSynth::SelectIdealState( void )
{
	switch ( m_NPCState )
	{
	case NPC_STATE_COMBAT:
		{
			if ( GetEnemy() == NULL )
			{
				if ( !HasCondition( COND_ENEMY_DEAD ) )
				{
					...
					SetCondition( COND_COMBINE_SYNTH_SHOULD_PATROL );
				}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::OnBeginMoveAndShoot()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineSynth::OnEndMoveAndShoot()
{
	VacateStrategySlot();
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
WeaponProficiency_t CNPC_CombineSynth::CalcWeaponProficiency( CBaseCombatWeapon *pWeapon )
{
	...
	{
		if( m_nSkin != COMBINE_SYNTH_SKIN_SHOTGUNNER )
		{
			m_nSkin = COMBINE_SYNTH_SKIN_SHOTGUNNER;
		}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::HasShotgun()
{
	if( GetActiveWeapon() && GetActiveWeapon()->m_iClassname == s_iszShotgunSynthClassname )
	{
		return true;
	}
...
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::ActiveWeaponIsFullyLoaded()
{
...
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter *sourceEnt)
{
	if ( interactionType == g_interactionTurretStillStanding )
	{
		...
		if ( sourceEnt == GetEnemy() )
		{
			...
			SetCondition( COND_COMBINE_SYNTH_DROP_GRENADE );
...
//
//-----------------------------------------------------------------------------
const char* CNPC_CombineSynth::GetSquadSlotDebugName( int iSquadSlot )
{
	switch( iSquadSlot )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::IsUsingTacticalVariant( int variant )
{
...
//-----------------------------------------------------------------------------
bool CNPC_CombineSynth::IsRunningApproachEnemySchedule()
{
	...
	if( IsCurSchedule( SCHED_COMBINE_SYNTH_PRESS_ATTACK, false ) )
		return true;

	return false;
}

bool CNPC_CombineSynth::ShouldPickADeathPose( void ) 
{ 
	return !IsCrouching(); 
}
...
AI_BEGIN_CUSTOM_NPC( npc_COMBINE_SYNTH_base, CNPC_CombineSynth )

//Tasks
DECLARE_TASK( TASK_COMBINE_SYNTH_FACE_TOSS_DIR )
DECLARE_TASK( TASK_COMBINE_SYNTH_IGNORE_ATTACKS )
DECLARE_TASK( TASK_COMBINE_SYNTH_SIGNAL_BEST_SOUND )
DECLARE_TASK( TASK_COMBINE_SYNTH_DEFER_SQUAD_GRENADES )
DECLARE_TASK( TASK_COMBINE_SYNTH_CHASE_ENEMY_CONTINUOUSLY )
DECLARE_TASK( TASK_COMBINE_SYNTH_DIE_INSTANTLY )
DECLARE_TASK( TASK_COMBINE_SYNTH_PLAY_SEQUENCE_FACE_ALTFIRE_TARGET )
DECLARE_TASK( TASK_COMBINE_SYNTH_GET_PATH_TO_FORCED_GREN_LOS )
DECLARE_TASK( TASK_COMBINE_SYNTH_SET_STANDING )

//Activities
DECLARE_ACTIVITY( ACT_COMBINE_SYNTH_THROW_GRENADE )
DECLARE_ACTIVITY( ACT_COMBINE_SYNTH_LAUNCH_GRENADE )
DECLARE_ACTIVITY( ACT_COMBINE_SYNTH_BUGBAIT )
DECLARE_ACTIVITY( ACT_COMBINE_SYNTH_AR2_ALTFIRE )
DECLARE_ACTIVITY( ACT_SYNTH_WALK_EASY )
DECLARE_ACTIVITY( ACT_SYNTH_WALK_MARCH )

DECLARE_ANIMEVENT( COMBINE_SYNTH_AE_BEGIN_ALTFIRE )
DECLARE_ANIMEVENT( COMBINE_SYNTH_AE_ALTFIRE )

DECLARE_SQUADSLOT( SQUAD_SLOT_GRENADE1 )
DECLARE_SQUADSLOT( SQUAD_SLOT_GRENADE2 )

DECLARE_CONDITION( COND_COMBINE_SYNTH_NO_FIRE )
DECLARE_CONDITION( COND_COMBINE_SYNTH_DEAD_FRIEND )
DECLARE_CONDITION( COND_COMBINE_SYNTH_SHOULD_PATROL )
DECLARE_CONDITION( COND_COMBINE_SYNTH_HIT_BY_BUGBAIT )
DECLARE_CONDITION( COND_COMBINE_SYNTH_DROP_GRENADE )
...
(
 ...
 "	Tasks"
 "		 TASK_SET_FAIL_SCHEDULE				SCHEDULE:SCHED_COMBINE_SYNTH_RUN_AWAY_FROM_BEST_SOUND"
 "		 TASK_STOP_MOVING					0"
 "		 TASK_COMBINE_SYNTH_SIGNAL_BEST_SOUND		0"
 ...
 DEFINE_SCHEDULE	
 (
 SCHED_COMBINE_SYNTH_RUN_AWAY_FROM_BEST_SOUND,
 ...
 //=========================================================
 //	SCHED_COMBINE_SYNTH_COMBAT_FAIL
 //=========================================================
 DEFINE_SCHEDULE	
 (
 SCHED_COMBINE_SYNTH_COMBAT_FAIL,
 ...
 //=========================================================
 // SCHED_COMBINE_SYNTH_VICTORY_DANCE
 //=========================================================
 DEFINE_SCHEDULE	
 (
 SCHED_COMBINE_SYNTH_VICTORY_DANCE,
 ...
 "	Tasks "
 "		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_COMBINE_SYNTH_ESTABLISH_LINE_OF_FIRE"
 "		TASK_SET_TOLERANCE_DISTANCE		48"
 "		TASK_GET_PATH_TO_ENEMY_LKP		0"
 "		TASK_COMBINE_SYNTH_IGNORE_ATTACKS		0.2"
 "		TASK_SPEAK_SENTENCE				0"
 "		TASK_RUN_PATH					0"
 //		"		TASK_COMBINE_SYNTH_MOVE_AND_AIM		0"
 "		TASK_WAIT_FOR_MOVEMENT			0"
 "		TASK_COMBINE_SYNTH_IGNORE_ATTACKS		0.0"
 ""
 "	Interrupts "
 "		COND_NEW_ENEMY"
 "		COND_ENEMY_DEAD"
 ...
 DEFINE_SCHEDULE 
 (
 SCHED_COMBINE_SYNTH_ESTABLISH_LINE_OF_FIRE,

 "	Tasks "
 "		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_FAIL_ESTABLISH_LINE_OF_FIRE"
 "		TASK_SET_TOLERANCE_DISTANCE		48"
 "		TASK_GET_PATH_TO_ENEMY_LKP_LOS	0"
 "		TASK_COMBINE_SYNTH_SET_STANDING		1"
 ...
 //=========================================================
 // SCHED_COMBINE_SYNTH_PRESS_ATTACK
 //=========================================================
 DEFINE_SCHEDULE 
 (
 SCHED_COMBINE_SYNTH_PRESS_ATTACK,
 ...
 //=========================================================
 // SCHED_COMBINE_SYNTH_COMBAT_FACE
 //=========================================================
 DEFINE_SCHEDULE
 (
 SCHED_COMBINE_SYNTH_COMBAT_FACE,
 ...
 "		TASK_FACE_ENEMY				0"
 "		 TASK_WAIT					1.5"
 //"		 TASK_SET_SCHEDULE			SCHEDULE:SCHED_COMBINE_SYNTH_SWEEP"
 ""
 "	Interrupts"
 "		COND_NEW_ENEMY"
 "		COND_ENEMY_DEAD"
 ...
 DEFINE_SCHEDULE
 (
 SCHED_COMBINE_SYNTH_HIDE_AND_RELOAD,
 ...
 //=========================================================
 // SCHED_COMBINE_SYNTH_SIGNAL_SUPPRESS
 ...
 "		TASK_COMBINE_SYNTH_SET_STANDING		0"
 ...
 "		COND_HEAR_DANGER"
 "		COND_HEAR_MOVE_AWAY"
 "		COND_COMBINE_SYNTH_NO_FIRE"
 )

 //=========================================================
 // SCHED_COMBINE_SYNTH_SUPPRESS
 //=========================================================
 DEFINE_SCHEDULE	
 (
 SCHED_COMBINE_SYNTH_SUPPRESS,
 ...
 //=========================================================
 // 	SCHED_COMBINE_SYNTH_ENTER_OVERWATCH
 //
 ...
 "		TASK_SET_ACTIVITY			ACTIVITY:ACT_IDLE"
 "		TASK_FACE_ENEMY				0"
 "		TASK_SET_SCHEDULE			SCHEDULE:SCHED_COMBINE_SYNTH_OVERWATCH"
 ""
 "	Interrupts"
 "		COND_HEAR_DANGER"
 "		COND_NEW_ENEMY"
 ...
 //=========================================================
 // SCHED_COMBINE_SYNTH_WAIT_IN_COVER
 ...
 "		COND_HEAR_DANGER"
 "		COND_HEAR_MOVE_AWAY"
 "		COND_COMBINE_SYNTH_ATTACK_SLOT_AVAILABLE"
 )

 //=========================================================
 // SCHED_COMBINE_SYNTH_TAKE_COVER1
 //=========================================================
 DEFINE_SCHEDULE	
 (
 SCHED_COMBINE_SYNTH_TAKE_COVER1  ,
 ...
 "	Tasks"
 "		TASK_SET_FAIL_SCHEDULE		SCHEDULE:SCHED_COMBINE_SYNTH_TAKECOVER_FAILED"
 "		TASK_STOP_MOVING				0"
 "		TASK_WAIT					0.2"
 "		TASK_FIND_COVER_FROM_ENEMY	0"
 "		TASK_RUN_PATH				0"
 "		TASK_WAIT_FOR_MOVEMENT		0"
 "		TASK_REMEMBER				MEMORY:INCOVER"
 "		TASK_SET_SCHEDULE			SCHEDULE:SCHED_COMBINE_SYNTH_WAIT_IN_COVER"
 ""
 "	Interrupts"
 )
 ...
 DEFINE_SCHEDULE
 (
 SCHED_COMBINE_SYNTH_TAKECOVER_FAILED,
 ...
 //=========================================================
 // SCHED_COMBINE_SYNTH_GRENADE_COVER1
 //=========================================================
 DEFINE_SCHEDULE
 (
 SCHED_COMBINE_SYNTH_GRENADE_COVER1,
 ...
 //=========================================================
 // SCHED_COMBINE_SYNTH_TOSS_GRENADE_COVER1
 //
 ...
 //=========================================================
 // SCHED_COMBINE_SYNTH_RANGE_ATTACK1
 //=========================================================
 DEFINE_SCHEDULE
 (
 SCHED_COMBINE_SYNTH_RANGE_ATTACK1,
 ...
 "		TASK_RANGE_ATTACK1				0"
 "		TASK_COMBINE_SYNTH_IGNORE_ATTACKS		0.5"
 ""
 "	Interrupts"
 "		COND_NEW_ENEMY"
 "		COND_ENEMY_DEAD"
 ...
 DEFINE_SCHEDULE
 (
 SCHED_COMBINE_SYNTH_AR2_ALTFIRE,

 "	Tasks"
 "		TASK_STOP_MOVING									0"
 "		TASK_ANNOUNCE_ATTACK								1"
 "		TASK_COMBINE_SYNTH_PLAY_SEQUENCE_FACE_ALTFIRE_TARGET		ACTIVITY:ACT_COMBINE_SYNTH_AR2_ALTFIRE"
 ""
 "	Interrupts"
 )
 ...
 DEFINE_SCHEDULE
 (
 SCHED_COMBINE_SYNTH_FORCED_GRENADE_THROW,

 "	Tasks"
 "		TASK_STOP_MOVING					0"
 "		TASK_COMBINE_SYNTH_FACE_TOSS_DIR			0"
 "		TASK_ANNOUNCE_ATTACK				2"	// 2 = grenade
 "		TASK_PLAY_SEQUENCE					ACTIVITY:ACT_RANGE_ATTACK2"
 "		TASK_COMBINE_SYNTH_DEFER_SQUAD_GRENADES	0"
 ""
 "	Interrupts"
 )
 ...
 DEFINE_SCHEDULE
 (
 SCHED_COMBINE_SYNTH_MOVE_TO_FORCED_GREN_LOS,

 "	Tasks "
 "		TASK_SET_TOLERANCE_DISTANCE					48"
 "		TASK_COMBINE_SYNTH_GET_PATH_TO_FORCED_GREN_LOS	0"
 "		TASK_SPEAK_SENTENCE							1"
 "		TASK_RUN_PATH								0"
 "		TASK_WAIT_FOR_MOVEMENT						0"
 "	"
 ...
 //=========================================================
 // 	SCHED_COMBINE_SYNTH_RANGE_ATTACK2	
 //
 ...
 "		TASK_PLAY_SEQUENCE					ACTIVITY:ACT_RANGE_ATTACK2"
 "		TASK_COMBINE_SYNTH_DEFER_SQUAD_GRENADES	0"
 "		TASK_SET_SCHEDULE					SCHEDULE:SCHED_COMBINE_SYNTH_WAIT_IN_COVER"	// don't run immediately after throwing grenade.
 ""
 "	Interrupts"
 )
 ...
 DEFINE_SCHEDULE
 (
 SCHED_COMBINE_SYNTH_GRENADE_AND_RELOAD,
 ...
 "		TASK_WAIT						3"
 "		TASK_WAIT_RANDOM				3"
 "		TASK_SET_SCHEDULE				SCHEDULE:SCHED_COMBINE_SYNTH_PATROL" // keep doing it
 ""
 "	Interrupts"
 "		COND_ENEMY_DEAD"
 "		COND_LIGHT_DAMAGE"
 ...
 DEFINE_SCHEDULE
 (
 SCHED_COMBINE_SYNTH_BUGBAIT_DISTRACTION,

 "	Tasks"
 "		TASK_STOP_MOVING		0"
 "		TASK_RESET_ACTIVITY		0"
 "		TASK_PLAY_SEQUENCE		ACTIVITY:ACT_COMBINE_SYNTH_BUGBAIT"
 ""
 "	Interrupts"
 ""
 )
 ...
 //=========================================================
 // SCHED_COMBINE_SYNTH_CHARGE_TURRET
 //
 ...
 //=========================================================
 // SCHED_COMBINE_SYNTH_CHARGE_PLAYER
 //
 ...
 "		TASK_STOP_MOVING					0"
 "		TASK_SET_FAIL_SCHEDULE				SCHEDULE:SCHED_CHASE_ENEMY_FAILED"
 "		TASK_COMBINE_SYNTH_CHASE_ENEMY_CONTINUOUSLY		192"
 "		TASK_FACE_ENEMY						0"
 ""
 "	Interrupts"
 "		COND_NEW_ENEMY"
 ...
 //=========================================================
 // SCHED_COMBINE_SYNTH_DROP_GRENADE
 //
 ...
 //=========================================================
 // SCHED_COMBINE_SYNTH_PATROL_ENEMY
 //
 // Used instead if SCHED_COMBINE_SYNTH_PATROL if I have an enemy.
 ...
 DEFINE_SCHEDULE
 (
 SCHED_COMBINE_SYNTH_BURNING_STAND,

 "	Tasks"
 "		TASK_SET_ACTIVITY				ACTIVITY:ACT_COMBINE_SYNTH_BUGBAIT"
 "		TASK_RANDOMIZE_FRAMERATE		20"
 "		TASK_WAIT						2"
 "		TASK_WAIT_RANDOM				3"
 "		TASK_COMBINE_SYNTH_DIE_INSTANTLY		DMG_BURN"
 "		TASK_WAIT						1.0"
 "	"
 "	Interrupts"
 )
 ...
 DEFINE_SCHEDULE
 (
 SCHED_COMBINE_SYNTH_FACE_IDEAL_YAW,
 ...
 DEFINE_SCHEDULE
 (
 SCHED_COMBINE_SYNTH_MOVE_TO_MELEE,
