...
#define CREMATORRED_INJURED_HEALTH				100 //Pretty sure this is obsolete now. I'm leaving it in just in case


ConVar	sk_crematorred_health( "sk_crematorred_health","200");
ConVar	sk_crematorred_dmg_immo( "sk_crematorred_dmg_immo","1"); //Anything higher than this is OP
ConVar	sk_crematorred_max_range( "sk_crematorred_max_range","950");
ConVar	sk_crematorred_immolator_color_r ("sk_crematorred_immolator_color_r", "0");
ConVar	sk_crematorred_immolator_color_g ("sk_crematorred_immolator_color_g", "255");
ConVar	sk_crematorred_immolator_color_b ("sk_crematorred_immolator_color_b", "0");
ConVar	sk_crematorred_immolator_beamsprite ("sk_crematorred_immolator_beamsprite", "sprites/physbeam.vmt");

LINK_ENTITY_TO_CLASS( monster_crematorred, CNPC_CrematorRed );
LINK_ENTITY_TO_CLASS( npc_crematorred, CNPC_CrematorRed );

BEGIN_DATADESC( CNPC_CrematorRed )
	DEFINE_FIELD( m_flBurnRadius, FIELD_FLOAT ),
	DEFINE_FIELD( m_iBravery, FIELD_INTEGER ),
	DEFINE_ARRAY( m_pBeam, FIELD_CLASSPTR, CREMATORRED_MAX_BEAMS ),
	DEFINE_ARRAY( m_hNoise, FIELD_CLASSPTR, CREMATORRED_MAX_NOISE ),
...
enum
{
	SCHED_CREMATORRED_ATTACK = LAST_SHARED_SCHEDULE,
	SCHED_CREMATORRED_PATROL,
	SCHED_CREMATORRED_MOVE_DEPLOYED,
};

#define CREMATORRED_IGNORE_PLAYER 64

int ACT_RUN_DEPLOYED_CREMRED;
int ACT_WALK_DEPLOYED_CREMRED;
...
void CNPC_CrematorRed::Spawn()
{
	if ( sv_hl2_beta.GetFloat() == 1 )
	{
		...
		ClearEffects();
		m_iHealth			= sk_crematorred_health.GetFloat();
...
void CNPC_CrematorRed::Precache()
{
	...
	PrecacheModel( "models/gibs/crematorred_arm1.mdl" );
	PrecacheModel( "models/gibs/crematorred_arm2.mdl" );
	PrecacheModel( "models/gibs/crematorred_lfoot.mdl" );
	PrecacheModel( "models/gibs/crematorred_rfoot.mdl" );
	PrecacheModel( "models/gibs/crematorred_head.mdl" );
	PrecacheModel( "models/gibs/crematorred_midsection.mdl" );
...
void CNPC_CrematorRed::CallForHelp( char *szClassname, float flDist, CBaseEntity * pEnemy, Vector &vecLocation )
{
	...
	DevMsg( "CrematorRed: CALLING FOR BACKUP!\n" );
...
// ALert Sound
//=========================================================
void CNPC_CrematorRed::AlertSound( void )
{
	if ( GetEnemy() != NULL )
	{
	...
	/*	Vector vecTmp = GetEnemy()->GetAbsOrigin();
		CallForHelp( "npc_crematorred", 512, GetEnemy(), vecTmp );
...
// IdleSound
//=========================================================
void CNPC_CrematorRed::IdleSound( void )
{
...
// PainSound
//=========================================================
void CNPC_CrematorRed::PainSound( const CTakeDamageInfo &info )
{
	if ( random->RandomInt( 0, 2 ) == 0)
	{
...
void CNPC_CrematorRed::DeathSound( const CTakeDamageInfo &info )
{
	CPASAttenuationFilter filter( this );
	CSoundParameters params;
...
int CNPC_CrematorRed::GetSoundInterests ( void )
{
	return	SOUND_WORLD	|
			SOUND_COMBAT	|
			SOUND_DANGER	|
...
void CNPC_CrematorRed::Event_Killed( const CTakeDamageInfo &info )
{
...
//=========================================================
float CNPC_CrematorRed::MaxYawSpeed ( void )
{
	float flYS;
...
// HandleAnimEvent 
//=========================================================
void CNPC_CrematorRed::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
		...
		case CREM_AE_IMMO_SHOOT:
		{
			...
			te->DynamicLight( filter, 0.0, &GetAbsOrigin(), sk_crematorred_immolator_color_r.GetFloat(), sk_crematorred_immolator_color_g.GetFloat(), sk_crematorred_immolator_color_b.GetFloat(), 2, 256, 0.2 / m_flPlaybackRate, 0 );
...
// Output  :
//------------------------------------------------------------------------------
int CNPC_CrematorRed::RangeAttack1Conditions( float flDot, float flDist )
{
	if ( GetEnemy() == NULL )
		 return( COND_LOST_ENEMY );

	if ( flDist > sk_crematorred_max_range.GetFloat() ) //1024 by default
	{
...
void CNPC_CrematorRed::StartTask( const Task_t *pTask )
{
	ClearBeams();
...
int CNPC_CrematorRed::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	if ( info.GetAttacker()->GetTeamNumber() == 2 )
	{
		return 0;
	}
	else
	{
		...
		if( newInfo.GetDamageType() & DMG_DISSOLVE)
		{
			...
			DevMsg( "CrematorRed: Immolator damage, no actual damage is taken\n" );
		}
...
void CNPC_CrematorRed::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
...
//=========================================================
//=========================================================
int CNPC_CrematorRed::SelectSchedule( void )
{
	ClearBeams();
	...
	case NPC_STATE_COMBAT:
		...
		if ( m_iHealth < 20 || m_iBravery < 0)
			...
			if ( !HasCondition( COND_CAN_MELEE_ATTACK1 ) )
					...
					Vector vecTmp = GetEnemy()->GetAbsOrigin();
					CallForHelp( "monster_crematorred", 512, GetEnemy(), vecTmp );
					CallForHelp( "npc_crematorred", 512, GetEnemy(), vecTmp );
					CallForHelp( "npc_metropolice", 512, GetEnemy(), vecTmp );
					CallForHelp( "npc_combine_s", 512, GetEnemy(), vecTmp );
					CallForHelp( "npc_combineguard", 512, GetEnemy(), vecTmp );
	...
	case NPC_STATE_IDLE:
		...
		{
				return SCHED_CREMATORRED_PATROL;
		}
...
Activity CNPC_CrematorRed::NPC_TranslateActivity(Activity eNewActivity)
{
		if (m_iHealth <= sk_crematorred_health.GetFloat() / 2 && eNewActivity == ACT_RUN)
		{
			// limp!
	...
	/*	if ( GetEnemy() && eNewActivity == ACT_RUN)
		{
			return (Activity)ACT_RUN_DEPLOYED_CREMRED;
		} //Animation is selected ingame but does not work
		*/

		if (m_iHealth <= sk_crematorred_health.GetFloat() / 2 && eNewActivity == ACT_WALK )
		{
			// limp!
			return ACT_WALK_HURT;
		}
	/*	if ( GetEnemy() && eNewActivity == ACT_WALK )
		{
			return (Activity)ACT_WALK_DEPLOYED_CREMRED;
		}*/
...
int CNPC_CrematorRed::TranslateSchedule( int scheduleType )
{
	...
	case SCHED_RANGE_ATTACK1:
		{
			...
			return SCHED_CREMATORRED_ATTACK;
		}
...
void CNPC_CrematorRed::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
//=========================================================
void CNPC_CrematorRed::BeamGlow( )
{
	int b = m_iBeams * 32;
...
//=========================================================
void CNPC_CrematorRed::ImmoBeam( int side )
{
	Vector vecSrc, vecAim;
	trace_t tr;
	CBaseEntity *pEntity;

	if ( m_iBeams >= CREMATORRED_MAX_BEAMS )
		 return;
	if (m_iNoise >= CREMATORRED_MAX_NOISE )
		return;
	...
	UTIL_TraceLine ( vecSrc, vecSrc + vecAim * sk_crematorred_max_range.GetFloat(), MASK_SOLID, this, COLLISION_GROUP_NONE, &tr); //range is 1024 by default
	
	m_pBeam[m_iBeams] = CBeam::BeamCreate( sk_crematorred_immolator_beamsprite.GetString(), 5.0f ); //physbeam default
	...
	m_pBeam[m_iBeams]->SetColor( sk_crematorred_immolator_color_r.GetFloat(), sk_crematorred_immolator_color_g.GetFloat(), sk_crematorred_immolator_color_b.GetFloat() ); // 0 255 0 by default
	...
	for( m_iNoise = 0 ; m_iNoise < CREMATORRED_MAX_NOISE ; m_iNoise++ ) 
	{
		Vector vecDest;
		...
		m_hNoise[m_iNoise] = CBeam::BeamCreate(  sk_crematorred_immolator_beamsprite.GetString(), 3.00f ); //physbeam default
		...
		m_hNoise[m_iNoise]->SetColor( sk_crematorred_immolator_color_r.GetFloat(), sk_crematorred_immolator_color_g.GetFloat(), sk_crematorred_immolator_color_b.GetFloat() ); //0 255 0
	...
	{
		CTakeDamageInfo info( this, this, sk_crematorred_dmg_immo.GetFloat(), DMG_DISSOLVE ); //dmg_dissolve and dmg_burn by default
		Ignite(100);
		CalculateMeleeDamageForce( &info, vecAim, tr.endpos );
		RadiusDamage( CTakeDamageInfo( this, this, sk_crematorred_dmg_immo.GetFloat(), DMG_BURN ), tr.endpos, 128,  CLASS_NONE, NULL ); //changed from 256 to 128 to correspond with noisebeams
...
//=========================================================
void CNPC_CrematorRed::ClearBeams( )
{
	for (int i = 0; i < CREMATORRED_MAX_BEAMS; i++)
	{
		if (m_pBeam[i])
		{
	...
	for (int i =0; i < CREMATORRED_MAX_NOISE; i++)
	{
		if (m_hNoise[i])
		{
...
AI_BEGIN_CUSTOM_NPC( npc_crematorred, CNPC_CrematorRed )

	DECLARE_ACTIVITY(ACT_WALK_DEPLOYED_CREMRED);
	DECLARE_ACTIVITY(ACT_RUN_DEPLOYED_CREMRED);

	//=========================================================
	// > SCHED_CREMATORRED_ATTACK
	//=========================================================
	DEFINE_SCHEDULE
	(
		SCHED_CREMATORRED_ATTACK,
	...
	SCHED_CREMATORRED_PATROL,
		...
		"		TASK_WAIT						3"
		"		TASK_WAIT_RANDOM				3"
		"		TASK_SET_SCHEDULE				SCHEDULE:SCHED_CREMATORRED_PATROL" // keep doing it
		""
		 "	Interrupts"
		 "		COND_ENEMY_DEAD"
		 "		COND_LIGHT_DAMAGE"
