...
#define ROLLERMINERED_MAX_TORQUE_FACTOR	5
extern short g_sModelIndexWExplosion;

ConVar	sk_rollerminered_shock( "sk_rollerminered_shock","0");
ConVar	sk_rollerminered_stun_delay("sk_rollerminered_stun_delay", "1");
ConVar	sk_rollerminered_vehicle_intercept( "sk_rollerminered_vehicle_intercept","1");
...
//-----------------------------------------------------------------------------
// CRollerRedController implementation
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
class CRollerRedController : public IMotionEvent
{
...
BEGIN_SIMPLE_DATADESC( CRollerRedController )
...
//-----------------------------------------------------------------------------
IMotionEvent::simresult_e CRollerRedController::Simulate( IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse &angular )
{
	if( m_fIsStopped )
	{
		return SIM_NOTHING;
...
#define ROLLERMINERED_IDLE_SEE_DIST					2048
#define ROLLERMINERED_NORMAL_SEE_DIST					2048
#define ROLLERMINERED_WAKEUP_DIST						256
#define ROLLERMINERED_SEE_VEHICLESONLY_BEYOND_IDLE		300		// See every other than vehicles upto this distance (i.e. old idle see dist)
#define ROLLERMINERED_SEE_VEHICLESONLY_BEYOND_NORMAL	800		// See every other than vehicles upto this distance (i.e. old normal see dist)

#define ROLLERMINERED_RETURN_TO_PLAYER_DIST			(200*200)

#define ROLLERMINERED_MIN_ATTACK_DIST	1
#define ROLLERMINERED_MAX_ATTACK_DIST	4096

#define	ROLLERMINERED_OPEN_THRESHOLD	256

#define ROLLERMINERED_VEHICLE_OPEN_THRESHOLD	400
#define ROLLERMINERED_VEHICLE_HOP_THRESHOLD	300

#define ROLLERMINERED_HOP_DELAY				2			// Don't allow hops faster than this

//#define ROLLERMINERED_REQUIRED_TO_EXPLODE_VEHICLE		4

#define ROLLERMINERED_FEAR_DISTANCE			(300*300)
...
enum
{
	SCHED_ROLLERMINERED_RANGE_ATTACK1 = LAST_SHARED_SCHEDULE,
	SCHED_ROLLERMINERED_CHASE_ENEMY,
	SCHED_ROLLERMINERED_BURIED_WAIT,
	SCHED_ROLLERMINERED_BURIED_UNBURROW,
	SCHED_ROLLERMINERED_FLEE,
	SCHED_ROLLERMINERED_ALERT_STAND,
	SCHED_ROLLERMINERED_NUDGE_TOWARDS_NODES,
	SCHED_ROLLERMINERED_PATH_TO_PLAYER,
	SCHED_ROLLERMINERED_ROLL_TO_PLAYER,
	SCHED_ROLLERMINERED_POWERDOWN,
};
...
enum 
{
	TASK_ROLLERMINERED_CHARGE_ENEMY = LAST_SHARED_TASK,
	TASK_ROLLERMINERED_BURIED_WAIT,
	TASK_ROLLERMINERED_UNBURROW,
	TASK_ROLLERMINERED_GET_PATH_TO_FLEE,
	TASK_ROLLERMINERED_NUDGE_TOWARDS_NODES,
	TASK_ROLLERMINERED_RETURN_TO_PLAYER,
	TASK_ROLLERMINERED_POWERDOWN,
};
...
#define ROLLERMINERED_SE_CLEAR		0x00000000
#define ROLLERMINERED_SE_CHARGE	0x00000001
#define ROLLERMINERED_SE_TAUNT		0x00000002
#define ROLLERMINERED_SE_SHARPEN	0x00000004
#define ROLLERMINERED_SE_TOSSED	0x00000008

enum rollingsoundstate_t { ROLL_SOUND_NOT_READY = 0, ROLL_SOUND_OFF, ROLL_SOUND_CLOSED, ROLL_SOUND_OPEN };

//=========================================================
//=========================================================
class CNPC_RollerMineRed : public CNPCBaseInteractive<CAI_BaseNPC>, public CDefaultPlayerPickupVPhysics
{
	DECLARE_CLASS( CNPC_RollerMineRed, CNPCBaseInteractive<CAI_BaseNPC> );
	DECLARE_SERVERCLASS();

public:

	CNPC_RollerMineRed( void ) { m_bTurnedOn = true; m_bUniformSight = false; }
	~CNPC_RollerMineRed( void );
	...
	Vector EyePosition() 
	{
		...
		// that the abs origin is at the center of the rollerminered
	...
	{ 
		...
		return ( m_bHeld || m_bHackedByAlyx ) ? CLASS_HACKED_ROLLERMINE : CLASS_PLAYER_ALLY; 
	}
	...
	int			CountRollersOnMyVehicle( CUtlVector<CNPC_RollerMineRed*> *pRollerList );
...
protected:
	...
	CRollerRedController			m_RollerController;
...
string_t CNPC_RollerMineRed::gm_iszDropshipClassname;

LINK_ENTITY_TO_CLASS( npc_rollerminered, CNPC_RollerMineRed );

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
BEGIN_DATADESC( CNPC_RollerMineRed )
...
IMPLEMENT_SERVERCLASS_ST( CNPC_RollerMineRed, DT_RollerMineRed )
...
bool NPC_RollerMineRedredred_IsRollerMineRedredred( CBaseEntity *pEntity )
{
	CNPC_RollerMineRed *pRoller = dynamic_cast<CNPC_RollerMineRed *>(pEntity);
	return pRoller ? true : false;
}

CBaseEntity *NPC_RollerMineRedredred_DropFromPoint( const Vector &originStart, CBaseEntity *pOwner, const char *pszTemplate )
{
	CBaseEntity *pEntity = NULL;
	CNPC_RollerMineRed *pMine = NULL;

	// Use the template, if we have it
	if ( pszTemplate && pszTemplate[0] )
	{
		MapEntity_ParseEntity( pEntity, pszTemplate, NULL );
		pMine = dynamic_cast<CNPC_RollerMineRed *>(pEntity);
	}
	else
	{
		pMine = (CNPC_RollerMineRed*)CreateEntityByName("npc_rollerminered");
	}
	...
	{
		Warning( "NULL Ent in RollerMineRedredred Create!\n" );
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
CNPC_RollerMineRed::~CNPC_RollerMineRed( void )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::Precache( void )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::Spawn( void )
{
	Precache();
	...
	SetDistLook( ROLLERMINERED_IDLE_SEE_DIST );

	if( m_bUniformSight )
	{
		m_flSeeVehiclesOnlyBeyond = ROLLERMINERED_IDLE_SEE_DIST;
	}
	else
	{
		m_flSeeVehiclesOnlyBeyond = ROLLERMINERED_SEE_VEHICLESONLY_BEYOND_IDLE;
	}
...
//-----------------------------------------------------------------------------
unsigned int CNPC_RollerMineRed::PhysicsSolidMaskForEntity( void ) const 
{ 
	if ( HasSpawnFlags( SF_ROLLERMINERED_PROP_COLLISION ) )
		return MASK_SOLID;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::Bury( trace_t *tr )
{
	...
	SetSchedule( SCHED_ROLLERMINERED_BURIED_WAIT );
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_RollerMineRed::WakeupMine( CAI_BaseNPC *pNPC )
{
	if ( pNPC && pNPC->m_iClassname == m_iClassname && pNPC != this )
	{
		CNPC_RollerMineRed *pMine = dynamic_cast<CNPC_RollerMineRed *>(pNPC);
		if ( pMine )
		{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::WakeNeighbors()
{
	...
	Vector range(ROLLERMINERED_WAKEUP_DIST,ROLLERMINERED_WAKEUP_DIST,64);
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::OnStateChange( NPC_STATE OldState, NPC_STATE NewState )
{
	if ( NewState == NPC_STATE_IDLE )
	{
		SetDistLook( ROLLERMINERED_IDLE_SEE_DIST );
		m_flDistTooFar = ROLLERMINERED_IDLE_SEE_DIST;
	...
	else
	{
		...
		SetDistLook( ROLLERMINERED_NORMAL_SEE_DIST );

		if( m_bUniformSight )
		{
			m_flSeeVehiclesOnlyBeyond = ROLLERMINERED_NORMAL_SEE_DIST;
		}
		else
		{
			m_flSeeVehiclesOnlyBeyond = ROLLERMINERED_SEE_VEHICLESONLY_BEYOND_NORMAL;
		}

		m_flDistTooFar = ROLLERMINERED_NORMAL_SEE_DIST;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
NPC_STATE CNPC_RollerMineRed::SelectIdealState( void )
{
	switch ( m_NPCState )
	{
...
//-----------------------------------------------------------------------------
bool CNPC_RollerMineRed::BecomePhysical( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::OnRestore()
{
	BaseClass::OnRestore();
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_RollerMineRed::CreateVPhysics()
{
	if ( m_bBuried )
	{
		VPhysicsInitStatic();
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_RollerMineRed::RangeAttack1Conditions( float flDot, float flDist )
{
	...
	if( flDist > ROLLERMINERED_MAX_ATTACK_DIST  )
		return COND_TOO_FAR_TO_ATTACK;
	
	if (flDist < ROLLERMINERED_MIN_ATTACK_DIST )
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_RollerMineRed::SelectSchedule( void )
{
	if ( m_bPowerDown )
		return SCHED_ROLLERMINERED_POWERDOWN;

	if ( m_bBuried )
	{
		if ( HasCondition(COND_NEW_ENEMY) || HasCondition(COND_LIGHT_DAMAGE) )
			return SCHED_ROLLERMINERED_BURIED_UNBURROW;

		return SCHED_ROLLERMINERED_BURIED_WAIT;
	}
	...
	if ( HasCondition( COND_SEE_FEAR ) )
		return SCHED_ROLLERMINERED_FLEE;
	...
	case NPC_STATE_COMBAT:
		...
		if ( HasCondition( COND_CAN_RANGE_ATTACK1 ) )
			return SCHED_ROLLERMINERED_RANGE_ATTACK1;
		
		return SCHED_ROLLERMINERED_CHASE_ENEMY;
		break;
	...
	// RollerMineRedredreds never wait to fall to the ground
...
// Purpose: 
//-----------------------------------------------------------------------------
int CNPC_RollerMineRed::GetHackedIdleSchedule( void )
{
	...
	if ( !HasCondition(COND_SEE_PLAYER) )
		return SCHED_ROLLERMINERED_PATH_TO_PLAYER;

	if ( GetAbsOrigin().DistToSqr( pPlayer->GetAbsOrigin() ) > ROLLERMINERED_RETURN_TO_PLAYER_DIST )
		return SCHED_ROLLERMINERED_ROLL_TO_PLAYER;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_RollerMineRed::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
	...
	case SCHED_ALERT_STAND:
		{
			...
			return SCHED_ROLLERMINERED_ALERT_STAND;
		}
		break;

	case SCHED_ROLLERMINERED_RANGE_ATTACK1:
		if( HasCondition(COND_ENEMY_OCCLUDED) )
		{
			// Because of an unfortunate arrangement of cascading failing schedules, the rollerminered
			...
			return SCHED_ROLLERMINERED_NUDGE_TOWARDS_NODES;
		}
		break;
	}
...
#if 0
#define	ROLLERMINERED_DETECTION_RADIUS		350
//-----------------------------------------------------------------------------
// Purpose: 
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_RollerMineRed::DetectedEnemyInProximity( void )
{
	CBaseEntity *pEnt = NULL;
	CBaseEntity *pBestEnemy = NULL;
	float		flBestDist = MAX_TRACE_LENGTH;

	while ( ( pEnt = gEntList.FindEntityInSphere( pEnt, GetAbsOrigin(), ROLLERMINERED_DETECTION_RADIUS ) ) != NULL )
	{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_RollerMineRed::QuerySeeEntity(CBaseEntity *pSightEnt, bool bOnlyHateOrFearIfNPC)
{
	if ( IRelationType( pSightEnt ) == D_FR )
	{
		...
		if ( flDist > ROLLERMINERED_FEAR_DISTANCE )
			return false;
	}
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::StartTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	...
	case TASK_ROLLERMINERED_UNBURROW:
	...
	case TASK_ROLLERMINERED_BURIED_WAIT:
	...
	case TASK_WAIT_FOR_MOVEMENT:
		{
			// TASK_RUN_PATH and TASK_WALK_PATH work different on the rollerminered and run until movement is done,
	...
	case TASK_ROLLERMINERED_CHARGE_ENEMY:
	case TASK_ROLLERMINERED_RETURN_TO_PLAYER:
		{
			IPhysicsObject *pPhysicsObject = VPhysicsGetObject();
	...
	case TASK_ROLLERMINERED_GET_PATH_TO_FLEE:
		{
			// Find the nearest thing we're afraid of, and move away from it.
			float flNearest = ROLLERMINERED_FEAR_DISTANCE;
	...
	case TASK_ROLLERMINERED_NUDGE_TOWARDS_NODES:
		{
			IPhysicsObject *pPhysicsObject = VPhysicsGetObject();
	...
	case TASK_ROLLERMINERED_POWERDOWN:
		break;
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::RunTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	case TASK_ROLLERMINERED_UNBURROW:
		{	
			...
			// Robin: HACK: Bloat the rollerminered check to catch the model switch (roller.mdl->roller_spikes.mdl)
			trace_t	tr;
	...
	case TASK_WALK_PATH:
		...
		if ( IsCurSchedule(SCHED_ROLLERMINERED_FLEE) && !HasCondition( COND_SEE_FEAR ) )
		{
			TaskComplete();
			break;
		}
...
//				Assert(0);
			...
			float flMaxTorque = ROLLERMINERED_MAX_TORQUE_FACTOR;
			
			// Friendly rollerminereds go a little slower
			if ( HasSpawnFlags( SF_ROLLERMINERED_FRIENDLY ) )
			{
				flMaxTorque *= 0.75;
			}
			...
			if( !(m_iSoundEventFlags & ROLLERMINERED_SE_TAUNT) && UTIL_DistApprox( GetLocalOrigin(), vecTargetPosition ) <= 400 )
			{
				m_iSoundEventFlags |= ROLLERMINERED_SE_TAUNT; // Don't repeat.

				EmitSound( "NPC_RollerMine.Taunt" );
			}

			// Jump earlier when chasing a vehicle
			float flThreshold = ROLLERMINERED_OPEN_THRESHOLD;
			if ( EnemyInVehicle() )
			{
				flThreshold = ROLLERMINERED_VEHICLE_OPEN_THRESHOLD;
			}
			...
			else if ( m_bIsOpen )
			{
				...
				else if ( EnemyInVehicle() && flDistance < ROLLERMINERED_VEHICLE_HOP_THRESHOLD )
				{
	...
	case TASK_ROLLERMINERED_RETURN_TO_PLAYER:
		{
			...
			float flMaxTorque = ROLLERMINERED_MAX_TORQUE_FACTOR * 0.75;
			if( flTorqueFactor < 1 )
			{
				flTorqueFactor = 1;
			}
			...
			if ( GetAbsOrigin().DistToSqr( vecTargetPosition ) < (ROLLERMINERED_RETURN_TO_PLAYER_DIST*2.0) )
			{
				TaskComplete();
			}
		}
	...
	case TASK_ROLLERMINERED_POWERDOWN:
		{
			...
			{
				SetThink( &CNPC_RollerMineRed::PreDetonate );
				SetNextThink( gpGlobals->curtime + 0.5f );
			}

			// No TaskComplete() here, because the task will never complete. The rollerminered will explode.
		}
		break;	
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::Open( void )
{
	...
	if ( m_bIsOpen == false )
	{
		...
		SetTouch( &CNPC_RollerMineRed::ShockTouch );
		m_bIsOpen = true;
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::SetRollerSkin( void )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::Close( void )
{
	...
	if ( m_bIsOpen && !IsShocking() )
	{
		...
		m_iSoundEventFlags = ROLLERMINERED_SE_CLEAR;
	}
}
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::SpikeTouch( CBaseEntity *pOther )
{
	/*
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::CloseTouch( CBaseEntity *pOther )
{
	if ( pOther->IsSolidFlagSet(FSOLID_TRIGGER | FSOLID_VOLUME_CONTENTS) )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::EmbedTouch( CBaseEntity *pOther )
{
	if ( pOther->IsSolidFlagSet(FSOLID_TRIGGER | FSOLID_VOLUME_CONTENTS) )
		return;
	...
	if ( pOther->entindex() == 0 )
	{
		...
		UTIL_DecalTrace( &tr, "RollerMineRedredred.Crater" );
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_RollerMineRed::IsPlayerVehicle( CBaseEntity *pEntity )
{
...
// Output : float
//-----------------------------------------------------------------------------
float CNPC_RollerMineRed::GetAttackDamageScale( CBaseEntity *pVictim )
{
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::ShockTarget( CBaseEntity *pOther )
{
	CBeam *pBeam;

	if( m_bHackedByAlyx )
	{
		pBeam = CBeam::BeamCreate( "sprites/rollerminered_shock_yellow.vmt", 4 );
	}
	else
	{
		pBeam = CBeam::BeamCreate( "sprites/rollerminered_shock.vmt", 4 );
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::NotifyInteraction( CAI_BaseNPC *pUser )
{
	...
	// Force the rollerminered open here. At very least, this ensures that the 
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::ShockTouch( CBaseEntity *pOther )
{
	if ( pOther->IsSolidFlagSet(FSOLID_TRIGGER | FSOLID_VOLUME_CONTENTS) )
		return;
	...
	// If we're a 'friendly' rollerminered, just push the player a bit
	if ( HasSpawnFlags( SF_ROLLERMINERED_FRIENDLY ) )
	{
		if ( pOther->IsPlayer() )
		{
	...
	SetTouch( &CNPC_RollerMineRed::CloseTouch );
	Vector vel;
	...
	CTakeDamageInfo	info( this, this, vecForce, out, sk_rollerminered_shock.GetFloat(), DMG_SHOCK );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	if ( m_bHeld || m_hVehicleStuckTo || !IsPlayerVehicle( pOther ) )
	{
		...
		if ( m_bEmbedOnGroundImpact )
		{
			...
			g_PostSimulationQueue.QueueCall( this, &CNPC_RollerMineRed::EmbedTouch, pOther );
		}
		return;
	}
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::StickToVehicle( CBaseEntity *pOther )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
int CNPC_RollerMineRed::CountRollersOnMyVehicle( CUtlVector<CNPC_RollerMineRed*> *pRollerList )
{
	CBaseEntity *entityList[64];
...
//-----------------------------------------------------------------------------
// Purpose: Tell other rollerminereds on the vehicle that I've just arrived
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::AnnounceArrivalToOthers( CBaseEntity *pOther )
{
	// Now talk to any other rollerminereds stuck to the same vehicle
	CUtlVector<CNPC_RollerMineRed*> aRollersOnVehicle;
	...
	/*
	if ( iRollers >= ROLLERMINERED_REQUIRED_TO_EXPLODE_VEHICLE )
	{
		// Alert the others
...
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::InputConstraintBroken( inputdata_t &inputdata )
{
	// Prevent rollerminereds being dislodged right as they stick
...
//-----------------------------------------------------------------------------
// Purpose: Respond to another rollerminered that's chirped at us
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::InputRespondToChirp( inputdata_t &inputdata )
{
	EmitSound( "NPC_RollerMine.ChirpRespond" );
}

//-----------------------------------------------------------------------------
// Purpose: Respond to another rollerminered's signal to detonate
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::InputRespondToExplodeChirp( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::InputJoltVehicle( inputdata_t &inputdata )
{
	...
	/*
	CBeam *pBeam = CBeam::BeamCreate( "sprites/rollerminered_shock.vmt", 4 );
	if ( pBeam )
	{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::InputTurnOn( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::InputTurnOff( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::InputPowerdown( inputdata_t &inputdata )
{
	m_bPowerDown = true;
...
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::UnstickFromVehicle( void )
{
	if ( m_pConstraint )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
CBaseEntity *CNPC_RollerMineRed::GetVehicleStuckTo( void )
{
	if ( !m_pConstraint )
		return NULL;
...
// Input  : *pPhysGunUser - 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::OnPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
...
// Input  : *pPhysGunUser - 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::OnPhysGunDrop( CBasePlayer *pPhysGunUser, PhysGunDrop_t Reason )
{
	m_bHeld = false;
	m_flActiveTime = gpGlobals->curtime + GetStunDelay();
	m_RollerController.On();
	...
	if ( Reason == LAUNCHED_BY_CANNON )
	{
		...
		{
			//m_bIsPrimed = true;
			SetTouch( &CNPC_RollerMineRed::SpikeTouch );
...
// Output : float
//-----------------------------------------------------------------------------
int CNPC_RollerMineRed::OnTakeDamage( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::Hop( float height )
{
	...
	if ( GetMoveType() == MOVETYPE_VPHYSICS )
	{
		...
		m_flNextHop = gpGlobals->curtime + ROLLERMINERED_HOP_DELAY;
	}
}
...
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::PreDetonate( void )
{
	Hop( 300 );

	SetTouch( NULL );
	SetThink( &CNPC_RollerMineRed::Explode );
	SetNextThink( gpGlobals->curtime + 0.5f );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::Explode( void )
{
	...
	SetThink( &CNPC_RollerMineRed::SUB_Remove );
...
// Purpose: 
//-----------------------------------------------------------------------------
float CNPC_RollerMineRed::RollingSpeed()
{
	IPhysicsObject *pPhysics = VPhysicsGetObject();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CNPC_RollerMineRed::GetStunDelay()
{
	if( m_bHackedByAlyx )
	{
		return 0.1f;
	...
	{
		return sk_rollerminered_stun_delay.GetFloat();
	}
}
...
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::EmbedOnGroundImpact()
{
	m_bEmbedOnGroundImpact = true;

	SetTouch( &CNPC_RollerMineRed::EmbedTouch );
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::PrescheduleThink()
{
	// Are we underwater?
	if ( UTIL_PointContents( GetAbsOrigin() ) & MASK_WATER )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::UpdateRollingSound()
{
...
void CNPC_RollerMineRed::StopRollingSound()
{
	CSoundEnvelopeController &controller = CSoundEnvelopeController::GetController();
...
void CNPC_RollerMineRed::UpdatePingSound()
{
	float pingSpeed = 0;
	if ( m_bIsOpen && !IsShocking() && !m_bHeld )
	{
		...
		if ( pEnemy )
		{
			...
			pingSpeed = clamp( pingSpeed, 1, ROLLERMINERED_OPEN_THRESHOLD );
			pingSpeed *= (1.0f/ROLLERMINERED_OPEN_THRESHOLD);
		}
	}
...
void CNPC_RollerMineRed::StopPingSound()
{
	CSoundEnvelopeController &controller = CSoundEnvelopeController::GetController();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::StopLoopingSounds( void )
{
	StopRollingSound();
	StopPingSound();
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_RollerMineRed::IsValidEnemy( CBaseEntity *pEnemy )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_RollerMineRed::EnemyInVehicle( void )
{
	...
	if ( pCCEnemy != NULL && pCCEnemy->IsInAVehicle() )
		return ( sk_rollerminered_vehicle_intercept.GetBool() );
...
// Purpose: 
//-----------------------------------------------------------------------------
float CNPC_RollerMineRed::VehicleHeading( CBaseEntity *pVehicle )
{
...
//			*ptr - 
//-----------------------------------------------------------------------------
void CNPC_RollerMineRed::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
...
AI_BEGIN_CUSTOM_NPC( npc_rollerminered, CNPC_RollerMineRed )

	//Tasks
	DECLARE_TASK( TASK_ROLLERMINERED_CHARGE_ENEMY )
	DECLARE_TASK( TASK_ROLLERMINERED_BURIED_WAIT )
	DECLARE_TASK( TASK_ROLLERMINERED_UNBURROW )
	DECLARE_TASK( TASK_ROLLERMINERED_GET_PATH_TO_FLEE )
	DECLARE_TASK( TASK_ROLLERMINERED_NUDGE_TOWARDS_NODES )
	DECLARE_TASK( TASK_ROLLERMINERED_RETURN_TO_PLAYER )
	DECLARE_TASK( TASK_ROLLERMINERED_POWERDOWN )
	...
	SCHED_ROLLERMINERED_BURIED_WAIT,
		...
		"	Tasks"
		"		TASK_ROLLERMINERED_BURIED_WAIT		0"
		"	"
		"	Interrupts"
		"		COND_NEW_ENEMY"
		"		COND_LIGHT_DAMAGE"
	...
	DEFINE_SCHEDULE
	(
	SCHED_ROLLERMINERED_RANGE_ATTACK1,

		"	Tasks"
		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_CHASE_ENEMY"
		"		TASK_ROLLERMINERED_CHARGE_ENEMY	0"
		"	"
		"	Interrupts"
		"		COND_ENEMY_DEAD"
		"		COND_NEW_ENEMY"
	...
	SCHED_ROLLERMINERED_CHASE_ENEMY,
		...
		"	Tasks"
		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_ROLLERMINERED_RANGE_ATTACK1"
	...
	SCHED_ROLLERMINERED_FLEE,
		...
		"		TASK_ROLLERMINERED_GET_PATH_TO_FLEE	300"
		"		TASK_RUN_PATH						0"
		"		TASK_STOP_MOVING					0"
		"	"
		"	Interrupts"
	...
	"	Tasks"
	"		TASK_ROLLERMINERED_NUDGE_TOWARDS_NODES		0"
	"		TASK_WAIT								1.5"
	""
	"	Interrupts"
	""
	...
	(
		...
		"	Tasks"
		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_ROLLERMINERED_ALERT_STAND"
	...
	(
		...
		"		TASK_SET_FAIL_SCHEDULE				SCHEDULE:SCHED_ROLLERMINERED_ALERT_STAND"
		"		TASK_SET_TOLERANCE_DISTANCE			200"
		"		TASK_ROLLERMINERED_RETURN_TO_PLAYER	0"
		""
		"	Interrupts"
		"		COND_NEW_ENEMY"
		"		COND_SEE_ENEMY"
	...
	(
		...
		"		TASK_STOP_MOVING			0"
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_IDLE"
		"		TASK_ROLLERMINERED_POWERDOWN	0"
		""
		"	Interrupts"
		""
	);
